...
public class SWTGraphics2D extends Graphics2D {
	...
	public GraphicsConfiguration getDeviceConfiguration() {
		...
		if(this.deviceConfiguration == null) {
			...
			this.deviceConfiguration = new SWTGraphicsConfiguration(width, height);
		}
		return this.deviceConfiguration;
	}

	/**
	 * Returns the current value for the specified hint key, or {@code null} if no
	 * value is set.
	 *
	 * @param hintKey
	 *            the hint key ({@code null} permitted).
	 *
	 ...
	 *             if {@code hintValue} is not compatible with
	 *             the {@code hintKey}.
	 *
	 ...
	 * case there is no real gradient support, the paint used is the {@code Color}
	 * returned by {@code getColor1()}).
	 *
	 * @param paint
	 *            the paint ({@code null} permitted, ignored).
	 *
	 * @see #getPaint()
	 * @see #setColor(Color)
	 */
	...
	/**
	 * Fill a rectangle area on the SWT graphic composite. The {@code fillRectangle}
	 * method of the {@code GC} class uses the background color so we must switch
	 * colors.
	 * 
	...
	public FontMetrics getFontMetrics(Font font) {
		...
		org.eclipse.swt.graphics.Font swtFont = getSwtFontFromPool(font);
		org.eclipse.swt.graphics.Font saved = gc.getFont();
		gc.setFont(swtFont);
		final org.eclipse.swt.graphics.FontMetrics m = gc.getFontMetrics();
		final GC measureGc = this.gc;
		gc.setFont(saved);
		return new FontMetrics(font) {
			...
			public int stringWidth(String s) {
				...
				return measureGc.textExtent(s).x;
			}

			public int charWidth(char c) {

				return measureGc.textExtent(String.valueOf(c)).x;
			}
		};
	}
	...
	public void drawString(String text, float x, float y) {
		...
		org.eclipse.swt.graphics.Font swtFont = getSwtFontFromPool(this.awtFont);
		this.gc.setFont(swtFont);
		int ascent = this.gc.getFontMetrics().getAscent();
		this.gc.drawString(text, (int)x, (int)(y - ascent), true);
	}
	 ...
	 * shape (the interior, if {@code onStroke} is false, otherwise the stroked
	 ...
	 *            the image.
	 * @param op
	 *            the operation ({@code null} permitted).
	 * @param x
	 *            the x-coordinate.
	 * @param y
	 *            the y-coordinate.
	...
	/**
	 * Draws a rendered image. If {@code img} is {@code null} this method does
	 * nothing.
	 *
	 * @param image
	 *            the rendered image ({@code null} permitted).
	 * @param xform
	 *            the transform.
	 */
	@Override
	...
	public boolean drawImage(Image image, int dx1, int dy1, int dx2, int dy2, int sx1, int sy1, int sx2, int sy2, ImageObserver observer) {
		...
		org.eclipse.swt.graphics.Image im = new org.eclipse.swt.graphics.Image(this.gc.getDevice(), data);
		this.gc.drawImage(im, sx1, sy1, sx2, sy2, dx1, dy1, w, h);
		im.dispose();
		// BufferedImage img2 = new BufferedImage(w, h, BufferedImage.TYPE_INT_ARGB);
	 ...
	 * instead of creating a new one. This is used in {@link #setFont(Font)} for
	 * instance.
	 *
	 * @param font
	 ...
	 * instead of creating a new one. This is used in {@link #setColor(Color)} for
	 * instance.
	 *
	 * @param awtColor
	...
	private org.eclipse.swt.graphics.Color getSwtColorFromPool(Color awtColor) {
		...
		org.eclipse.swt.graphics.Color swtColor = (org.eclipse.swt.graphics.Color)this.colorsPool.get(Integer.valueOf(awtColor.getRGB()));
		if(swtColor == null) {
	...
	/**
	 * Fetches an SWT Pattern matching the supplied gradient paint, or creates one,
	 * and returns it.
	 *
	 * @param gp
	 *            the gradient paint.
	...
	private Pattern fetchOrCreateSWTPattern(GradientPaint gp) {
		...
		if(swtPattern == null) {
			swtPattern = new Pattern(this.gc.getDevice(), (float)gp.getPoint1().getX(), (float)gp.getPoint1().getY(), (float)gp.getPoint2().getX(), (float)gp.getPoint2().getY(), getSwtColorFromPool(gp.getColor1()), getSwtColorFromPool(gp.getColor2()));
