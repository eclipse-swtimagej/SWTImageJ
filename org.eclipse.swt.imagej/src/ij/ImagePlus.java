...
public class ImagePlus implements ImageObserver, Measurements, Cloneable {
	...
	protected Image img;
	protected org.eclipse.swt.graphics.Image imgSwt;
	...
	/**
	 * Constructs an ImagePlus from an SWT Image. The first argument will be used as
	 * the title of the window that displays the image. Throws an
	 * IllegalStateException if an error occurs while loading the image.
	 */
	public ImagePlus(String title, org.eclipse.swt.graphics.Image image) {
	...
	/*
	 * To be implemented - called by setImage - setImageSwt calls waitForImage AWT
	 * below!
	 */
	private void waitForImageSwt(org.eclipse.swt.graphics.Image image) {

	}

	/* Also called by setImageSwt! */
	...
	public void draw() {
		...
		/* Changed for SWT. The Split Channels action causes a thread blocking! */
		if(!Thread.currentThread().getName().equals("Split Channels")) {
			if(win != null)
	...
	public void draw(int x, int y, int width, int height) {
		...
		if(win != null) {
			...
			float aspectRatioX = ic.aspectRatioX;
			float aspectRatioY = ic.aspectRatioY;
			width = (int)(width * mag);
			height = (int)(height * mag);
			if(aspectRatioX == 1.0f && aspectRatioY == 1.0f) {
				/* Repaint calls a wrapped SWT redraw! */
				ic.repaint(x, y, width, height);
			}
			/* We simply draw all if we have an embedded composite! */
			else {
				ic.repaint();
			}
	...
	public void show(String statusMessage) {
		...
		if((IJ.isMacro() && ij == null) || Interpreter.isBatchMode()) {
			if(isComposite())
				((CompositeImage)ImagePlus.this).reset();
		...
		imgSwt = getSwtImage();
		// img = getImage();
		if((imgSwt != null) && (width >= 0) && (height >= 0)) {
			activated = false;
			int stackSize = getStackSize();
			if(stackSize > 1) {
				Display.getDefault().syncExec(() -> {
					win = new StackWindow(ImagePlus.this); // displays the window and (if macro) waits for window to
															// be activated
				});
			} else if(getProperty(Plot.PROPERTY_KEY) != null) {
				Display.getDefault().syncExec(() -> {
					win = new PlotWindow(ImagePlus.this, (Plot)(getProperty(Plot.PROPERTY_KEY)));
				});
			} else {
				Display.getDefault().syncExec(() -> {
					win = new ImageWindow(ImagePlus.this);
				});
			}
			if(roi != null)
	...
	/**
	 * Returns a copy of this image as an 8-bit or RGB SWT image.
	 * 
	 * @see ij.process.ShortProcessor#get16BitBufferedImage
	 */
	// Changed for SWT. Created a ColorProcessor Constructor!
	public org.eclipse.swt.graphics.Image getImageSwt() {

		if(isComposite()) {
			ColorProcessor colorProcessor = new ColorProcessor(getSwtImage());
			// ColorProcessor colorProcessor = new ColorProcessor(getImage());
			return colorProcessor.getSwtImage();
		} else
			return ip.getSwtImage();
	}
	...
	public void setImage(Image image) {
		...
		if(image instanceof BufferedImage) {
			...
			int type = bi.getType();
			boolean rgb = type == BufferedImage.TYPE_3BYTE_BGR || type == BufferedImage.TYPE_INT_RGB || type == BufferedImage.TYPE_4BYTE_ABGR || type == BufferedImage.TYPE_BYTE_INDEXED || type == BufferedImage.TYPE_INT_ARGB || type == BufferedImage.TYPE_INT_BGR;
			int dataType = bi.getSampleModel().getDataType();
			if(IJ.debugMode)
				IJ.log("setImage: type=" + type + ", bands=" + nBands + ", rgb=" + rgb + ", datatype=" + dataType);
			if(nBands > 1 && !rgb) {
	 ...
	 * IllegalStateException if an error occurs while loading the image.
	 * 
	 * This is a SWT help implementation with Buffered image of one type! It would
	 * be nice if we have more options for a pure SWT implementation. In SWT the RGB
	 * type and number of bands is not available and we have to find out what we can
	 * use instead.
	 */
	public void setImageSwt(org.eclipse.swt.graphics.Image imag) {

		ImageData data = imag.getImageData();
		/*
		 * PaletteData palette = data.palette; Indexed palette! if (palette.isDirect) {
		 * if(data.depth==8) { } } Direct palette else { }
		 */
		/* Changed to convert SWT to a typed BufferedImage by default! */
		BufferedImage image = SWTUtils.convertToAWT(data, BufferedImage.TYPE_INT_RGB);
		if(image instanceof BufferedImage) {
			BufferedImage bi = (BufferedImage)image;
			int nBands = bi.getSampleModel().getNumBands();
			int type = bi.getType();
			boolean rgb = type == BufferedImage.TYPE_3BYTE_BGR || type == BufferedImage.TYPE_INT_RGB || type == BufferedImage.TYPE_4BYTE_ABGR || type == BufferedImage.TYPE_BYTE_INDEXED || type == BufferedImage.TYPE_INT_ARGB || type == BufferedImage.TYPE_CUSTOM;
			if(IJ.debugMode)
	 ...
	 * @return
	 */
	public static ImageProcessor convertToImageProcessor(org.eclipse.swt.graphics.Image img, int band) {

		ImageData data = img.getImageData();
		PaletteData palette = img.getImageData().palette;
		int w = data.width;
		int h = data.height;
		int dataType = data.depth;
		// Read data as float (no matter what it is - it's the most accuracy ImageJ can
		// provide)
		FloatProcessor fp = new FloatProcessor(w, h);
		float[] pixels = (float[])fp.getPixels();
		BufferedImage im = SWTUtils.convertToAWT(data);
		im.getRaster().getSamples(0, 0, w, h, band, pixels);
		// int[] pixels = (byte[]) data.data;
	...
	public void setStack(String title, ImageStack newStack) {
		...
		if(newStackSize > 1 && !(win instanceof StackWindow)) {
			...
			win = new StackWindow(this, dimensionsChanged ? null : canvas); // replaces this window
			if(IJ.isMacro())
	 ...
	 */
	/*
	 * public ImageCanvas getCanvas() { return win != null ? win.getCanvas() :
	 * flatteningCanvas; }
	 */
	...
	public ImageCanvas getCanvas() {
		...
		return win != null ? win.getImageCanvas() : flatteningCanvas;
	}
	 ...
	 * histogram, pixelCount, mean, mode, standard deviation, min and max.
	 */
	...
	/**
	 * Displays the specified stack image, where {@literal 1<=n<=stackSize}. Does
	 * nothing if this image is not a stack.
	 * 
	 * @see #setPosition
	 * @see #setC
	 * @see #setZ
	...
	/**
	 * Displays the cursor coordinates and ROI width and height on the canvas.
	 * Called by the class Roi in method 'showStatus'.
	 */
	public void showOnCanvasStatus(int x, int y, String s) {
	...
	/*
	 * Called by the ImageWindow class which get the mouse event from the
	 * ImageCanvas class!
	 */
	...
	/**
	 * Uses the specified method to set the threshold levels of this image. Use
	 * AutoThresholder.getMethods() to get a list of the available methods. Add "
	 * dark" to the method name if the image has a dark background. Add " 16-bit" to
	 * use the full 16-bit histogram when calculating the threshold of 16-bit
	 * images. Add " stack" to use the histogram of the entire stack when
	 ...
	 * This is not ported to SWT - To Do. Still misses a canvas when creating an
	 * ImageCanvas!
	 */
	public ImagePlus flatten() {
		...
		dis.syncExec(new Runnable() {
			...
			public void run() {
				...
				if(IJ.debugMode)
					IJ.log("flatten");
				ImagePlus impCopy = ImagePlus.this;
				if(getStackSize() > 1)
				 ...
				 * BufferedImage.TYPE_INT_ARGB); Graphics2D g = (Graphics2D)bi.getGraphics();
				 * g.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
				 * antialiasRendering?RenderingHints.VALUE_ANTIALIAS_ON:RenderingHints.
				 * VALUE_ANTIALIAS_OFF); g.drawImage(getImage(), 0, 0, null); ic2.paint(g);
				 */
				org.eclipse.swt.graphics.Image img = new org.eclipse.swt.graphics.Image(Display.getDefault(), width, height);
				GC gc = new GC(img);
				if(antialiasRendering) {
					gc.setAntialias(SWT.ON);
				} else {
					gc.setAntialias(SWT.OFF);
				}
				gc.drawImage(getSwtImage(), 0, 0);
				/* For SWT we paint on the GC with a separate method! */
				ic2.paint(gc);
				/*
				 * org.eclipse.swt.widgets.Event e = new org.eclipse.swt.widgets.Event(); e.gc =
				 * gc; org.eclipse.swt.graphics.Rectangle bounds = img.getBounds(); e.height =
				 * bounds.height; e.width = bounds.width; e.x = 0; e.y = 0; PaintEvent
				 * paintEvent = new PaintEvent(e); ic2.paint(paintEvent);
				 */
				// ImageLoader loader = new ImageLoader();
				// loader.data = new ImageData[] {img.getImageData()};
				// loader.save("exportedImage.bmp", SWT.IMAGE_BMP);
				// loader.save("C:/Users/Marcel/Downloads/test.jpg",SWT.IMAGE_JPEG);
				// gc.dispose();disposed in the paint(gc) method!
				imp2.flatteningCanvas = null;
				imp3 = new ImagePlus("Flat_" + getTitle(), img);
				imp3.copyScale(ImagePlus.this);
	...
	public void setEditorParent(org.eclipse.swt.widgets.Composite parent) {

		editorParent = parent;
	}

	public org.eclipse.swt.widgets.Composite getEditorParent() {
