...
public class ImportDialog {
	...
	void openAll(String directory, String[] list, FileInfo fi) {
		...
		if(stack != null) {
			...
			if(imp.getBitDepth() == 16 || imp.getBitDepth() == 32 || imp.getBitDepth() == 64)
	...
	private void guessFormat(String dir, String name) {
		...
		if(fileSize == 8 * imageSize)
			choiceSelection = indexOfType("64-bit Real");
		else if(fileSize == 4 * imageSize)
			choiceSelection = indexOfType("32-bit Real");
		else if(fileSize == 2 * imageSize)
			choiceSelection = indexOfType("16-bit Unsigned");
		else if(fileSize == 3 * imageSize)
			choiceSelection = indexOfType("24-bit RGB");
		else if(fileSize == imageSize)
			choiceSelection = indexOfType("8-bit");
	...
	private int indexOfType(String label) {
