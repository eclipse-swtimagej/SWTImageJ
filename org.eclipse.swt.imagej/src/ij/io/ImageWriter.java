...
public class ImageWriter {
	...
	void writeFloatStack(OutputStream out, Object[] stack) throws IOException {
		...
		for(int i = 0; i < fi.nImages; i++) {
			IJ.showStatus("Writing: " + (i + 1) + "/" + fi.nImages);
			Object pix = stack[i]; // audit-ok: GRAY32_FLOAT path; usually float[], may be double[] from a DoubleProcessor slice
			if(pix instanceof float[]) {
				writeFloatImage(out, (float[])pix);
			} else if(pix instanceof double[]) {
				// In-memory stack contains a DoubleProcessor slice inside a GRAY32_FLOAT save:
				// downcast to float[] so the on-disk format stays 32-bit float.
				double[] dpix = (double[])pix;
				float[] fpix = new float[dpix.length];
				for(int k = 0; k < dpix.length; k++)
					fpix[k] = (float)dpix[k];
				writeFloatImage(out, fpix);
			} else {
				throw new IOException("ImageWriter: unexpected pixel type for GRAY32_FLOAT stack at slice " + (i + 1) + ": " + (pix == null ? "null" : pix.getClass().getName()));
			}
			IJ.showProgress((double)(i + 1) / fi.nImages);
		}
	}
	...
	void writeFloatVirtualStack(OutputStream out, VirtualStack virtualStack) throws IOException {
		...
		for(int i = 1; i <= fi.nImages; i++) {
			...
			if(flip)
				ip.flipVertical();
			Object pix = ip.getPixels(); // audit-ok: GRAY32_FLOAT path; usually float[], may be double[] from a DoubleProcessor slice
			if(pix instanceof float[]) {
				writeFloatImage(out, (float[])pix);
			} else if(pix instanceof double[]) {
				// VirtualStack returned a DoubleProcessor inside a GRAY32_FLOAT save:
				// downcast to float[] so the on-disk format stays 32-bit float.
				double[] dpix = (double[])pix;
				float[] fpix = new float[dpix.length];
				for(int k = 0; k < dpix.length; k++)
				...
				throw new IOException("ImageWriter: unexpected pixel type for GRAY32_FLOAT virtual stack: " + (pix == null ? "null" : pix.getClass().getName()));
			}
			IJ.showProgress((double)i / fi.nImages);
		}
	}
