...
public class ResultsTable implements Cloneable {
	...
	public static TextWindow getResultsWindow() {
		Object f = WindowManager.getWindow("Results");
		if (f == null || !(f instanceof TextWindow))
			return null;
		else
			return (TextWindow) f;
	 ...
	 * <code>show()</code> must be called to update the window that displays the
	 * table. If the array is shorter than the column length, the remaining values
	 * of the column are left unchanged. If the array is longer, the table is
	 * extended. String values are unaffected, but only used if the numeric value at
	 * the given position is NaN.
	 */
	...
	public void show(String windowTitle) {
		...
		if (windowTitle.equals("Results")) {
			...
			Object frame = WindowManager.getWindow(windowTitle);
			TextWindow win;
	...
	public static boolean selectRow(Roi roi) {
		...
		if (name == null || name.length() > 8)
			return false;
		Object frame = WindowManager.getWindow("Results");
		if (frame == null)
			return false;
