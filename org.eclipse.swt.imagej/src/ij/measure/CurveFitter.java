...
public class CurveFitter implements UserFunction {
	...
	public static final int STRAIGHT_LINE = 0, POLY2 = 1, POLY3 = 2, POLY4 = 3, EXPONENTIAL = 4, POWER = 5, LOG = 6,
			...
			POLY7 = 19, POLY8 = 20, GAUSSIAN_NOOFFSET = 21, EXP_RECOVERY_NOOFFSET = 22, CHAPMAN = 23, ERF = 24;
	...
	public static final int[] sortedTypes = { STRAIGHT_LINE, POLY2, POLY3, POLY4, POLY5, POLY6, POLY7, POLY8, POWER,
			...
			LOG2, GAUSSIAN, GAUSSIAN_NOOFFSET, ERF, RODBARD, RODBARD2, INV_RODBARD, GAMMA_VARIATE, CHAPMAN };
	...
	public static final String[] fitList = { "Straight Line", "2nd Degree Polynomial", "3rd Degree Polynomial",
			...
			"Exponential Recovery (no offset)", "Chapman-Richards", "Error Function" }; // fList, doFit(),
																						// getNumParams() and
																						// makeInitialParamsAndVariations()
	...
	public static final String[] fMacro = { "y = a+x*b", "y = a+x*(b+x*c)", // STRAIGHT_LINE,POLY2
			"y = a+x*(b+x*(c+x*d))", "y = a+x*(b+x*(c+x*(d+x*e)))", "y = a*Math.exp(b*x)", "y = a*Math.pow(x,b)",
			"y = a*Math.log(b*x)", // EXPONENTIAL,POWER,LOG
	 ...
	 * showSettings is true, pops up a dialog allowing the user to set the initial
	 * fit parameters and various numbers controlling the Minimizer Returns the
	 * number of parameters, or 0 in case of a macro syntax error.
	 *
	 ...
	 * doCustomFit method with a (java) UserFunction, which also has more options.
	 *
	 */
	 ...
	 * setOffsetMultiplySlopeParams, to avoid searching for one or two parameters
	 * that can be calculated directly by linear regression.
	 *
	 * @param userFunction           A class instance implementing the userFunction
	 *                               interface. There, the fit function hould be
	 *                               defined by the method userFunction(params, x).
	 ...
	 *                               function with these parameters must not return
	 *                               NaN for any of the data points given in the
	 *                               constructor (xData). initialParams may be null,
	 *                               then random values are used, with repeated
	 *                               tries if the userFunction returns NaN.
	 ...
	 *                               Minimizer.minimize for details. Providing this
	 *                               array is especially valuable if one or more
	 *                               initial parameters have a value of 0.
	 ...
	 * length as the data arrays passed with the constructor. If the error bars of
	 * the data points are known, the weights should be proportional to 1/error^2.
	...
	public static double f(int fitType, double[] p, double x) {
		...
		case POLY8:
			return p[0] + x
					* (p[1] + x * (p[2] + x * (p[3] + x * (p[4] + x * (p[5] + x * (p[6] + x * (p[7] + x * p[8])))))));
		case EXPONENTIAL:
		case EXP_REGRESSION:
	 ...
	 * properly i.e., if getStatus() returns Minimizer.INITILIZATION_FAILURE). If
	 * weights have been specified, each of the residuals is multiplied by the
	 ...
	 * where n is the number of points. If weights are provided, the standard
	 * deviation does not take the weights into account. With weights, the standard
	 * deviation and getSumResidualsSqr (which uses weights) are not related the
	 * usual way.
	 */
	public double getSD() {
	 ...
	 * number of points is much larger than the number of fit parameters. Assumes
	 * that the data points are independent (i.e., each point having a different x
	 * value). For power, exp by linear regression and 'Rodbard NIH Image', this is
	 * calculated for the fit actually done, not for the residuals of the original
	 * data.
	 */
	 ...
	 * that this is not neccessarily the equation acutally used for the fit (for the
	 * various "linear regression" types and RODBARD2, the fit is done differently).
	 * Note that no macro code may be avialable for custom fits using the
	...
	private void doRegression(double[] params) {
		...
		if (offsetParam < 0) { // only 'factor' parameter, no offset:
			...
			sumResidualsSqr = sqr(factor) * sumX2 + sumWeights * sqr(offset) + sumY2 + 2 * factor * offset * sumX
	...
	private boolean makeInitialParamsAndVariations(int fitType) {
		...
		if (!hasInitialParams) {
			...
			case LOG2: // y = a+b*ln(x-c)
				initialParams[0] = yMean; // don't care, we will do this via regression
				initialParams[1] = (yMax - yMin + 1e-100) / (xMax - xMin + 1e-100); // don't care, we will do this via
																					// regression
			...
			case ERF: // a+b*erf((x-c)/d)
				...
				initialParams[1] = 0.5 * (yMax - yMin + 1e-100) * (lasty > firsty ? 1 : -1); // actually don't care, we
																								// will do this via
																								// regression
