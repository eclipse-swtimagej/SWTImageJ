...
public class CompositeImage extends ImagePlus {
	...
	org.eclipse.swt.graphics.Image rgbSwtImage;
	...
	@Override
	/*
	 * public org.eclipse.swt.graphics.Image getSwtImage(Display shellDisplay) { if
	 * (imgSwt==null) updateImage(); return swtImage; }
	 */
	...
	public void updateAndDraw() {
		...
		/* Changed for SWT. Else a deadlock occurs (thread is locking in synchronized method) */
		if(IJ.isMacro() == false)
			draw();
	}
	...
	public synchronized void updateImage() {
		...
		if(rgbPixels == null) {
			...
			rgbRaster = null;
			rgbImage = null;
			if(rgbSwtImage != null && !rgbSwtImage.isDisposed()) {
				rgbSwtImage.dispose();
			}
			rgbSwtImage = null;
		}
		...
		// createBufferedImage(); Changed for SWT!
		createImageSwt();
		if(img == null && awtImage != null) {
			img = awtImage;
		}
		if(img == null && swtImage != null) {
			imgSwt = swtImage;
		}
		singleChannel = false;
	}
			...
			for(int c = 0; c < nChn; c++) {
				switch(bitDepth) {
					...
					case 32:
						...
						break;
					case 64:
						v[c] = (int)Math.floor((in64[c][idx] - mins[c]) * scale[c]);
						break;
				}
	...
	void createSwtImage(Device device) {
		...
		if(imageSource == null) {
			...
			/* ImageData */
			ImageData currentImageData = new ImageData(width, height, 32, palette);
			currentImageData.setPixels(0, 0, width * height, rgbPixels, 0);
			// Constructs a new direct palette given the red, green and blue masks.
			swtImage = new org.eclipse.swt.graphics.Image(device, currentImageData);
			newPixels = false;
		} else if(newPixels) {
			imageSource.newPixels(rgbPixels, rgbCM, 0, width);
			newPixels = false;
	...
	void createImageSwt() {
		...
		/*
		 * if (rgbSampleModel == null) rgbSampleModel = getRGBSampleModel(); if
		 * (rgbRaster == null) { DataBuffer dataBuffer = new DataBufferInt(pixels, width
		 * * height, 0); rgbRaster = Raster.createWritableRaster(rgbSampleModel,
		 * dataBuffer, null); }
		 */
		/*
		 * if (imageSwt == null) { //Constructs a new direct palette given the red,
		 * green and blue masks. PaletteData palette = new PaletteData(0xFF0000, 0xFF00,
		 * 0xFF); ImageData data = new ImageData(width, height, 24, palette); int[]
		 * pixelArray = new int[3]; for (int y = 0; y < data.height; y++) { for (int x =
		 * 0; x < data.width; x++) { rgbRaster.getPixel(x, y, pixelArray); int pixel =
		 * palette.getPixel(new RGB(pixelArray[0], pixelArray[1], pixelArray[2]));
		 * data.setPixel(x, y, pixel); } } imageSwt = new
		 * org.eclipse.swt.graphics.Image(device, data); }
		 */
		// if (rgbRaster == null) {
		if(rgbSwtImage != null) {
			if(rgbSwtImage.isDisposed() == false) {
				rgbSwtImage.dispose();
			}
			rgbSwtImage = null;
		}
		...
		if(rgbSwtImage == null) {
			...
			PaletteData palette = new PaletteData(0xff0000, 0xff00, 0xff);
			/* ImageData */
			ImageData currentImageData = new ImageData(width, height, 24, palette);
			// ImageData currentImageData = new ImageData(width, height, 24,
			// new PaletteData(0xFF0000, 0x00FF00, 0x0000FF));
			currentImageData.setPixels(0, 0, width * height, rgbPixels, 0);
			// Constructs a new direct palette given the red, green and blue masks.
			rgbSwtImage = new org.eclipse.swt.graphics.Image(Display.getDefault(), currentImageData);
		}
		swtImage = rgbSwtImage;
	}
	...
	/*
	 * public Color getChannelColor() { if (lut==null || mode==GRAYSCALE) return
	 * Color.black; IndexColorModel cm = lut[getChannelIndex()]; if (cm==null)
	 * return Color.black; int index = cm.getMapSize() - 1; int r =
	 * cm.getRed(index); int g = cm.getGreen(index); int b = cm.getBlue(index);
	 * //IJ.log(index+" "+r+" "+g+" "+b); if (r<100 || g<100 || b<100) return new
	 * Color(r, g, b); else return Color.black; }
	 */

	/** Returns the color used to display the image subtitle and "B&C" histogram. */
	public org.eclipse.swt.graphics.Color getChannelColor() {
	 ...
	 * 'imp' is not a CompositeImage or 'imp' and this image do not have the same
	 * number of channels.
	 */
