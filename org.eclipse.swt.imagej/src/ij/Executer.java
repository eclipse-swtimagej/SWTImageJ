...
public class Executer implements Runnable {
	...
	boolean allowedWithLockedImage(String className) {
		return className.equals("ij.plugin.Zoom") || className.equals("ij.plugin.frame.ContrastAdjuster")
				|| className.equals("ij.plugin.SimpleCommands") || // includes Plugins>Utiltites>Reset (needed to reset
																	// a locked image)
				className.equals("ij.plugin.WindowOrganizer") || className.equals("ij.plugin.URLOpener");
	}
	...
	boolean openRecent(String cmd) {
		...
		for (int i = 0; i < menu.getItemCount(); i++) {
			if (menu.getItem(i).getText().equals(cmd)) {
				IJ.open(cmd);
				return true;
			}
		}
