...
public class ByteProcessor extends ImageProcessor {
	...
	/** Creates a ByteProcessor from an SWT Image. */
	public ByteProcessor(org.eclipse.swt.graphics.Image img) {
		...
		if (palette.isDirect) {
			cm = new DirectColorModel(data.depth, palette.redMask, palette.greenMask,
			          palette.blueMask);
			data.getPixels(0, 0, widthOfData, pixels, 0);
		} 
	...
	/**
	 * Creates a ByteProcessor from an ImageProcessor. 16-bit and 32-bit pixel data
	...
	synchronized org.eclipse.swt.graphics.Image createImageSwt() {
		...
		if (imageSwt == null || cm != cm2) {
			...
			/*
			 * PaletteData palette = new PaletteData(rgbs); ImageData data = new
			 * ImageData(getWidth(), getHeight(), colorModel.getPixelSize(), palette);
			 * data.transparentPixel = colorModel.getTransparentPixel(); // WritableRaster
			 * raster = bufferedImage.getRaster(); int[] pixelArray = new int[1]; for (int y
			 * = 0; y < data.height; y++) { for (int x = 0; x < data.width; x++) {
			 * raster.getPixel(x, y, pixelArray); data.setPixel(x, y, pixelArray[0]); } }
	 ...
	 * the image boundary. Values outside the range 0-255 are clamped.
	 */
	...
	/**
	 * Returns a reference to the byte array containing this image's pixel data. To
	...
	*/
	 ...
	 * (pixel values 0 to 255). The roi, mask, lut (ColorModel), threshold, min&max
	 * are also set for the FloatProcessor
	 * 
