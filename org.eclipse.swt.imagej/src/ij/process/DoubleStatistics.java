...
/** 64-bit (double) image statistics, including histogram. */
public class DoubleStatistics extends ImageStatistics {

	/** Constructs an ImageStatistics object from a DoubleProcessor
	    using the standard measurement options (area, mean, mode, min and max). */
	public DoubleStatistics(ImageProcessor ip) {
		this(ip, AREA + MEAN + MODE + MIN_MAX, null);
	}

	/** Constructs a DoubleStatistics object from a DoubleProcessor
	    using the specified measurement options. */
	public DoubleStatistics(ImageProcessor ip, int mOptions, Calibration cal) {
	...
	void getMedian(ImageProcessor ip, double minThreshold, double maxThreshold) {
		...
		double[] pixels = (double[]) ip.getPixels();
		double[] pixels2 = new double[pixelCount];
		byte[] mask = ip.getMaskArray();
		int i, mi;
		double v;
		int count = 0;
		...
		if ((pixels2.length & 1) == 0)
			median = (pixels2[middle - 1] + pixels2[middle]) / 2.0;
		else
