...
public class ShortProcessor extends ImageProcessor {
	...
	/**
	 * Creates a blank ShortProcessor using the default grayscale LUT that displays
	 * zero as black. Call invertLut() to display zero as white.
	 */
	...
	/** Creates a ShortProcessor from a TYPE_USHORT_GRAY BufferedImage. */
	public ShortProcessor(org.eclipse.swt.graphics.Image im) {
		ImageData dataIm = im.getImageData();
		if (dataIm.depth != 16)
			throw new IllegalArgumentException("Type!=TYPE_USHORT_GRAY");
		// API ImageData: Note that for 16 bit depth images the pixel data is stored in
		// least significant byte order; however, for 24bit and 32bit depth images the
	 ...
	 *             simulated by subtracting 32768 and using a calibration function
	 ...
	 * by subtracting 32768 and using a calibration function to restore the original
	 * values.
	 */
	...
	public org.eclipse.swt.graphics.Image createImageSwt() {
		...
		if (imageSwt == null || cm != cm2) {
			...
			/*
			 * int[] pixelArray = new int[1]; for (int y = 0; y < data.height; y++) { for
			 * (int x = 0; x < data.width; x++) { raster.getPixel(x, y, pixelArray);
	...
	/**
	 * Sets the min and max variables that control how real pixel values are mapped
	 * to 0-255 screen values. With signed 16-bit images, use
	 ...
	 * display. This ensures that this ShortProcessor is set up to correctly display
	 * the image.
	 */
	...
	/**
	 * Uses the current interpolation method (BILINEAR or BICUBIC) to calculate the
	 ...
	 * the image boundary. Values outside the range 0-65535 (-32768-32767 for signed
	 * images) are clipped. Support for signed values requires a calibration table,
	 * which is set up automatically with PlugInFilters.
	 */
	...
	/**
	 * Returns a reference to the short array containing this image's pixel data. To
	 * avoid sign extension, the pixel values must be accessed using a mask (e.g.
	...
	*/
	...
	/**
	 * 3x3 filter operations, code partly based on 3x3 convolution code contributed
	 * by Glynne Casteel.
	 */
	 ...
	 * selections, computations using these histograms are faster compared to 65536
	 * element histograms.
	 */
	int[] getHistogram2() {
		if (mask != null)
	...
	public void setThreshold(double minThreshold, double maxThreshold, int lutUpdate) {
		...
		// ij.IJ.log("setThreshold: "+lutUpdateMode+" "+this.minThreshold+"
		// "+this.maxThreshold);
	}
