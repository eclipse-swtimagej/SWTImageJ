...
public abstract class ImageProcessor implements Cloneable {
	...
	protected org.eclipse.swt.graphics.Image imageSwt;
	...
	/**
	 * Sets the color model. Must be an IndexColorModel (aka LUT) for all processors
	 * except the ColorProcessor.
	 */
	...
	/**
	 * Inverts the values in this image's LUT (indexed color model). Does nothing if
	 * this is a ColorProcessor.
	 */
	...
	/**
	 * Returns true if this image uses an inverting LUT that displays zero as white
	 * and 255 as black.
	 */
	...
	/**
	 * Returns 'true' if this is an image with a grayscale LUT or an RGB image with
	 * identical red, green and blue channels.
	 */
	...
	/**
	 * Sets the default fill/draw value to the pixel value closest to the specified
	 * color.
	 */
	public abstract void setColor(Color color);

	/**
	 * Sets the fill/draw color, where 'color' is "white", "black", "blue", etc., or
	...
	/**
	 * This image will be displayed by mapping pixel values in the range min-max to
	 * screen values in the range 0-255. For byte images, this mapping is done by
	 * updating the LUT. For short and float images, it's done by generating 8-bit
	 * AWT images. For RGB images, it's done by changing the pixel values. With
	...
	/**
	 * For short and float images, recalculates the min and max image values needed
	 * to correctly display the image. For ByteProcessors, resets the LUT.
	 */
	...
	/**
	 * Automatically sets the lower and upper threshold levels, where 'method' must
	 * be "Default", "Huang", "Intermodes", "IsoData", "IJ_IsoData", "Li",
	 * "MaxEntropy", "Mean", "MinError", "Minimum", "Moments", "Otsu", "Percentile",
	 * "RenyiEntropy", "Shanbhag", "Triangle" or "Yen". The 'method' string may also
	 * include the keywords 'dark' (dark background) 'red' (red LUT, the default),
	 * 'b&w' (black and white LUT), 'over/under' (over/under LUT) or 'no-lut' (no
	 * LUT changes), for example "Huang dark b&w". The display range of 16-bit and
	 * 32-bit images is not reset if the 'method' string contains 'no-reset'.
	 * 
	...
	/**
	 * Automatically sets the lower and upper threshold levels, where 'method' must
	 * be ISODATA or ISODATA2 and 'lutUpdate' must be RED_LUT, BLACK_AND_WHITE_LUT,
	 ...
	 * lutUpdateMode=NO_LUT_UPDATE. This removes the invisible threshold set by the
	 * MakeBinary and Convert to Mask commands.
	 * 
	 ...
	 * case for code executed in the <code>run()</code> method of plugins that
	...
	/**
	 * Defines a polygonal region of interest that will consist of a rectangular ROI
	 * and a mask. After processing, call <code>reset(mask)</code> to restore
	 * non-masked pixels. Here is an example:
	 * 
	 * <pre>
	...
	/**
	 * Use this method to set the interpolation method (NONE, BILINEAR or BICUBIC)
	 * used by scale(), resize() and rotate().
	 */
	 ...
	 * 8-connected.<br> Based on code from
	 * http://stackoverflow.com/questions/5186939/algorithm-for-drawing-a-4-
	 * connected-line<br> Author: Gabriel Landini (G.Landini at bham.ac.uk)
	 */
	 ...
	 * value. Draws multiple lines if the string contains newline characters. The
	 * current x coordinate is the left/center/right end of the string for
	 * left/center/right justification. The current y coordinate determines the
	 * bottommost position of the string, including the descent of the font (i.e.,
	 * characters reaching below the baseline) For multi-line strings, the y
	 * coordinate applies to the first line. The y of the drawing position is
	 * incremented by the height of one text line, i.e. points at the drawing
	 * position for the next text line
	 */
	...
	/**
	 * Draws a single-line string at the current drawing location cx, cy and adds
	 * the line height (FontMetrics.getHeight) to the current y coordinate 'cy' In
	 * SWT the font height for y alignment is incorrect!
	 */
	private void drawString2Swt(String s) {
		...
		GC g = new GC(bi);
		org.eclipse.swt.graphics.FontMetrics metrics = g.getFontMetrics();
		...
		g.setForeground(ij.swt.Color.white);
		g.fillRectangle(0, 0, w, h);
		g.setForeground(ij.swt.Color.black);
		g.drawString(s, 0, h - descent);
	...
	/**
	 * Draws a single-line string at the current drawing location cx, cy and adds
	 * the line height (FontMetrics.getHeight) to the current y coordinate 'cy'
	 */
	private void drawString2(String s) {
		...
		// if (ij.IJ.isMacOSX()) drawStringUsingGlyphVector(g, font, s, 0, h-descent);
		// else
		g.drawString(s, 0, h - descent);
		g.dispose();
		ImageProcessor ip = new ColorProcessor(bi);
	 ...
	 * RenderingHints.VALUE_ANTIALIAS_OFF); g.setFont(font); FontRenderContext frc =
	 * g.getFontRenderContext(); GlyphVector gv = font.createGlyphVector(frc, text);
	 * //Shape shape = gv.getOutline(); //g.translate(0, 10); //g.fill(shape);
	 * g.drawGlyphVector(gv, x, y); }
	 */

	/**
	 * Draws a string at the specified location using the current fill/draw value.
	 * Draws multiple lines if the string contains newline characters. The x
	 * coordinate is the left/center/right end of the string for left/center/right
	 * justification. The y coordinate determines the bottommost position of the
	 * string, including the descent of the font (i.e., characters reaching below
	 * the baseline) For multi-line strings, the y coordinate applies to the first
	 * line.
	 */
	...
	/**
	 * Specifies whether or not text is drawn using antialiasing, which is enabled
	 * by default on macOS or when the font size is 14 or greater. Antialiased text
	 * requires an 8 bit or RGB image. Antialiasing does not work with 8-bit images
	 ...
	 * necessarily including all whitespace at the right.
	 */
	...
	/**
	 * Returns a rectangle enclosing the pixels affected by drawString assuming it
	 * is drawn at (x=0, y=0). As drawString draws above the drawing location, the y
	 * coordinate of the rectangle is negative.
	 */
	 ...
	 * Use fill(Roi) or fill(ip.getMask()) to fill non-rectangular selections.
	 * 
	 * @see #setColor(Color)
	 * @see #setValue(double)
	 * @see #fill(Roi)
	...
	/**
	 * Fills pixels that are within the ROI bounding rectangle and part of the mask
	 * (i.e. pixels that have a value=BLACK in the mask array). Use ip.getMask() to
	 * acquire the mask. Throws and IllegalArgumentException if the mask is null or
	...
	/**
	 * Draws the specified ROI on this image using the line width and color defined
	 * by ip.setLineWidth() and ip.setColor().
	 * 
	...
	/**
	 * Draws the specified ROI on this image using the stroke width, stroke color
	 * and fill color defined by roi.setStrokeWidth, roi.setStrokeColor() and
	 * roi.setFillColor(). Works with RGB images. Does not work with 16-bit and
	 * float images.
	 * 
	...
	/**
	 * Draws the specified Overlay on this image. Works best with RGB images. Does
	 * not work with 16-bit and float images. Requires Java 1.6.
	 * 
	 * @see ImageProcessor#drawRoi
	 */
	...
	/**
	 * Set a lookup table used by getPixelValue(), getLine() and convertToFloat() to
	 * calibrate pixel values. The length of the table must be 256 for byte images
	 * and 65536 for short images. RGB and float processors do not do calibration.
	 * 
	...
	/**
	 * Set the range used for histograms of float images. The image range is used if
	 * both <code>histMin</code> and <code>histMax</code> are zero.
	 */
	...
	/**
	 * Returns the value of the pixel at (x,y). For RGB images, the argb values are
	 * packed in an int. For float images, the the value must be converted using
	 * Float.intBitsToFloat(). Returns zero if either the x or y coodinate is out of
	 * range. Use <i>getValue(x,y)</i> to get calibrated values from 8-bit and
	 * 16-bit images, to get intensity values from RGB images and to get float
	 * values from 32-bit images.
	 * 
	...
	/**
	 * This is a faster version of putPixel() that does not clip out of range values
	 * and does not do bounds checking.
	 */
	 ...
	 * getPixelValue() but does no bounds checking and does not return calibrated
	 * values.
	 */
	...
	/**
	 * Sets the value of the pixel at (x,y) to 'value'. Does no bounds checking or
	 * clamping, making it faster than putPixel(). Due to the lack of bounds
	 * checking, (x,y) coordinates outside the image may cause an exception. Due to
	 * the lack of clamping, values outside the 0-255 range (for byte) or 0-65535
	 * range (for short) are not handled correctly.
	 */
	 ...
	 * [x=0..width-1][y=0..height-1]. With RGB images, the returned values are in
	 * packed ARGB format. With float images, the returned values must be converted
	 ...
	 * three samples, all others have one. Returns zeros if the the coordinates are
	 * not in bounds. iArray is an optional preallocated array.
	 */
	...
	/**
	 * Uses the current interpolation method (bilinear or bicubic) to find the pixel
	 * value at real coordinates (x,y).
	 */
	 ...
	 * coordinates (x,y). For RGB images, the argb values are packed in an int. For
	 * float images, the value must be converted using Float.intBitsToFloat().
	...
	/**
	 * This method is from Chapter 16 of "Digital Image Processing: An Algorithmic
	 * Introduction Using Java" by Burger and Burge (http://www.imagingbook.com/).
	 */
	 ...
	 * image boundary. For 8-bit and 16-bit images, out of range values are clamped.
	 * For RGB images, the argb values are packed in 'value'. For float images,
	 * 'value' is expected to be a float converted to an int using
	 * Float.floatToIntBits().
	 */
	public abstract void putPixel(int x, int y, int value);

	/**
	 * Returns the value of the pixel at <i>(x,y)</i>, a calibrated value from 8-bit
	 * and 16-bit images, an intensity value from RGB images and a double value from
	 * 32-bit images. This is an alias for getPixelValue(x,y).
	 * 
	...
	/**
	 * Copies the image contained in 'ip' to (xloc, yloc) using one of the transfer
	 * modes defined in the Blitter interface.
	 */
	 ...
	 * must be 256 for byte images and 65536 for short images. RGB and float images
	 * are not supported.
	 */
	 ...
	 * <p>
	 * With 16-bit images, p=65535-p, or p=255-p, p=1024-p, etc. if an "Unsigned
	 * 16-bit range" is set using the "Set" option of the
	 * Image&gt;Adjust&gt;Brightness/Contrast dialog.
	 * <p>
	 * With 32-bit images, p=max-(p-min), where 'min' and 'max' are the minimum and
	 * maximum displayed pixel values.
	 */
	public void invert() {
	...
	/** Returns a copy of this image is the form of an SWT Image. */
	public abstract org.eclipse.swt.graphics.Image createSwtImage();
	...
	/** Returns this image as a SWT image. */
	/*
	 * public org.eclipse.swt.graphics.Image getSWTImage() { PaletteData palette =
	 * new PaletteData(0xFF, 0xFF00, 0xFF0000); ImageData imageData = new
	 * ImageData(width, height, 24, palette); org.eclipse.swt.graphics.Image im =
	 * new org.eclipse.swt.graphics.Image(Display.getDefault(), imageData); GC gc =
	 * new GC(im); gc.drawImage(createSwtImage(), 0, 0); gc.dispose(); return im; }
	 */
	...
	public org.eclipse.swt.graphics.Image getSwtImage() {
		...
		/*
		 * PaletteData palette = new PaletteData( 0xFF0000, 0xFF00, 0xFF ); ImageData
		 * data = new ImageData( width, height, 24, palette ); for ( int y = 0; y <
		 * data.height; y++ ) { for ( int x = 0; x < data.width; x++ ) {
		 * createSwtImage(); int rgba = img.getRGB( x, y ); int rgb = palette.getPixel(
		 * new RGB( ( rgba >> 16 ) & 0xFF, ( rgba >> 8 ) & 0xFF, rgba & 0xFF ) ); //int
		 * a = ( rgba >> 24 ) & 0xFF; data.setPixel( x, y, rgb ); //data.setAlpha( x, y,
		 * a ); } } return new org.eclipse.swt.graphics.Image( device, data);
		 */
		return createSwtImage();
	}
	...
	/**
	 * Restores pixels from the snapshot buffer that are within the rectangular roi
	 * but not part of the mask.
	 */
	...
	/**
	 * Adds pseudorandom, Gaussian ("normally") distributed values, with mean 0.0
	 * and the specified standard deviation, to this image or ROI.
	 */
	public abstract void noise(double standardDeviation);

	/**
	 * Returns a new processor containing an image that corresponds to the current
	 * ROI.
	 */
	public abstract ImageProcessor crop();

	/**
	 * Sets pixels less than 'level1' or greater than 'level2' to 0 and all other
	 * pixels to 255. Only works with 8-bit images.
	 */
	...
	/**
	 * Moves the image or selection vertically or horizontally by a specified number
	 * of pixels. Positive x values move the image or selection to the right,
	 * negative values move it to the left. Positive y values move the image or
	 * selection down, negative values move it up.
	 */
	...
	/**
	 * Returns the histogram of the image or ROI. Returns a luminosity histogram for
	 * RGB images and null for float images.
	 * <p>
	 * For 8-bit and 16-bit images, returns an array with one entry for each
	 * possible value that a pixel can have, from 0 to 255 (8-bit image) or 0-65535
	 * (16-bit image). Thus, the array size is 256 or 65536, and the bin width in
	 * uncalibrated units is 1.
	 * <p>
	 * For RGB images, the brightness is evaluated using the color weights (which
	 * would result in a float value) and rounded to an int. This gives 256 bins.
	 * FloatProcessor.getHistogram is not implemented (returns null).
	 */
	...
	/**
	 * Returns a 16-bit version of this image as a ShortProcessor. 32-bit pixel data
	 * are scaled from min-max to 0-255.
	 */
	...
	/**
	 * Returns a 16-bit version of this image as a ShortProcessor. 32-bit pixel data
	 * are scaled from min-max to 0-255 if 'scale' is true.
	 */
	 ...
	 * kernelHeight must be odd.
	 */
	public abstract void convolve(float[] kernel, int kernelWidth, int kernelHeight);

	/**
	 * Converts the image to binary using an automatically determined threshold. For
	 * byte and short images, converts to binary using an automatically determined
	 * threshold. For RGB images, converts each channel to binary. For float images,
	 * does nothing.
	 */
	 ...
	 * objects and background. It does this by taking a test threshold and computing
	 * the average of the pixels at or below the threshold and pixels above. It then
	 * computes the average of those two, increments the threshold, and repeats the
	 * process. Incrementing stops when the threshold is larger than the composite
	 * average. That is, threshold = (average background + average objects)/2. This
	 * description was posted to the ImageJ mailing list by Jordan Bevic.
	 */
	 ...
	 * drawPixel(). The clipping rectangle is reset by passing a null argument or by
	 * calling resetRoi().
	 */
	...
	/**
	 * This method is used by CompositeImage.updateImage() to create RGB images (for
	 * display) of a multi-channel composite images.
	 */
	...
	/**
	 * Sets the upper Over/Under threshold color. Can be called from a macro, e.g.,
	 * call("ij.process.ImageProcessor.setOverColor", 0,255,255).
	 */
	 ...
	 * including histogram, area, mean, min and max, standard deviation, and mode.
	 * Use the setRoi(Roi) method to limit statistics to a non-rectangular area.
	 * 
	...
	/**
	 * Calculates and returns complete uncalibrated (raw) statistics for this image
	 * or ROI but it is up to 70 times slower than getStats(). Use the setRoi(Roi)
	 * method to limit statistics to a non-rectangular area.
	 * 
	 * @return an {@link ij.process.ImageStatistics} object
	 * @see #setRoi(Roi)
	 * @see #getStats
	 ...
	 * VirtualStack.getProcessor(n) and the PlugInFilterRunner set the stack
	...
	/**
	 * Returns a shallow copy of this ImageProcessor, where this image and the copy
	 * share pixel data. Use the duplicate() method to create a copy that does not
	 * share the pixel data.
	 * 
	...
	/**
	 * Returns a color model based on the colors of the input from 0 to 254
	 * (inclusive). Entry 255 is red, for showing the thresholded areas.
	 */
