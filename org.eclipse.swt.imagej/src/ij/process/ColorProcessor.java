...
public class ColorProcessor extends ImageProcessor {
	...
	/** Creates a ColorProcessor from an SWT Image. */
	public ColorProcessor(org.eclipse.swt.graphics.Image img) {
		ImageData data_ = img.getImageData();
		width = data_.width;
		height = data_.height;
		pixels = new int[width * height];
		...
		ImageData data=imageScreen.getImageData();
		data.getPixels(0, 0, width * height, pixels, 0);
		//img.dispose();
		imageScreen.dispose();
		gc.dispose();
		
		/*Using just the 24-bit color model!*/
		createColorModel();
		fgColor = 0xff000000; // black
		resetRoi();
	}

	/** Creates a ColorProcessor from an SWT Image. PixelGrabber variant with SWT and AWT! */
	/*public ColorProcessor(org.eclipse.swt.graphics.Image img) {
		...
		height = data.height;
		pixels = new int[width * height];
		/*Here we create a BufferedImage to use the PixelGrabber class!
	...
	public org.eclipse.swt.graphics.Image createImageSwt() {
		...
		if (imageSwt == null) { // Constructs a new direct palette given the red,green and blue masks.
			PaletteData palette = new PaletteData(0xFF0000, 0xFF00, 0xFF);
			ImageData data = new ImageData(width, height, 24, palette);
			int[] pixelArray = new int[3];
			for (int y = 0; y < data.height; y++) {
				for (int x = 0; x < data.width; x++) {
					rgbRaster.getPixel(x, y, pixelArray);
		...
		if (imageSwt == null) {
			...
			ImageData currentImageData = new ImageData(width, height, 24,
	...
	/**
	 * Sets the default fill/draw value, where <code>value</code> is interpreted as
	 * an RGB int.
	 */
	...
	/**
	 * Sets the background fill value, where <code>value</code> is interpreted as an
	 * RGB int.
	 */
	...
	/**
	 * Stores the specified real grayscale value at (x,y). Does nothing if (x,y) is
	 * outside the image boundary. The value is clamped to be in the range 0-255.
	 */
	...
	/**
	 * Converts the specified pixel to grayscale using the formula g=(r+g+b)/3 and
	 * returns it as a float. Call setWeightingFactors() to specify different
	 * conversion factors.
	 */
	...
	/**
	 * Returns an ImageStack with three 8-bit slices, representing hue, saturation
	 * and brightness
	 */
	...
	/**
	 * Returns an ImageStack with three 32-bit slices, representing hue, saturation
	 * and brightness
	 */
	...
	public static final int RGB_NOISE = 0, RGB_MEDIAN = 1, RGB_FIND_EDGES = 2, RGB_ERODE = 3, RGB_DILATE = 4,
			RGB_THRESHOLD = 5, RGB_ROTATE = 6, RGB_SCALE = 7, RGB_RESIZE = 8, RGB_TRANSLATE = 9, RGB_MIN = 10,
			RGB_MAX = 11;
	...
	/**
	 * Uses averaging to creates a new ColorProcessor containing a downsized copy of
	 * this image or selection.
	 */
	...
	public void convolve3x3(int[] kernel) {
		...
		int k1 = kernel[0], k2 = kernel[1], k3 = kernel[2], k4 = kernel[3], k5 = kernel[4], k6 = kernel[5],
		...
		for (int y = yMin; y <= yMax; y++) {
			...
			for (int x = xMin; x <= xMax; x++) {
				...
				rsum = k1 * ((p1 & 0xff0000) >> 16) + k2 * ((p2 & 0xff0000) >> 16) + k3 * ((p3 & 0xff0000) >> 16)
						+ k4 * ((p4 & 0xff0000) >> 16) + k5 * ((p5 & 0xff0000) >> 16) + k6 * ((p6 & 0xff0000) >> 16)
						+ k7 * ((p7 & 0xff0000) >> 16) + k8 * ((p8 & 0xff0000) >> 16) + k9 * ((p9 & 0xff0000) >> 16);
				rsum /= scale;
				if (rsum > 255)
					rsum = 255;
				if (rsum < 0)
				...
				gsum = k1 * ((p1 & 0xff00) >> 8) + k2 * ((p2 & 0xff00) >> 8) + k3 * ((p3 & 0xff00) >> 8)
						+ k4 * ((p4 & 0xff00) >> 8) + k5 * ((p5 & 0xff00) >> 8) + k6 * ((p6 & 0xff00) >> 8)
						+ k7 * ((p7 & 0xff00) >> 8) + k8 * ((p8 & 0xff00) >> 8) + k9 * ((p9 & 0xff00) >> 8);
				gsum /= scale;
				if (gsum > 255)
					gsum = 255;
				else if (gsum < 0)
				...
				bsum = k1 * (p1 & 0xff) + k2 * (p2 & 0xff) + k3 * (p3 & 0xff) + k4 * (p4 & 0xff) + k5 * (p5 & 0xff)
						+ k6 * (p6 & 0xff) + k7 * (p7 & 0xff) + k8 * (p8 & 0xff) + k9 * (p9 & 0xff);
				bsum /= scale;
				if (bsum > 255)
					bsum = 255;
				if (bsum < 0)
				...
				pixels[offset++] = 0xff000000 | ((rsum << 16) & 0xff0000) | ((gsum << 8) & 0xff00) | (bsum & 0xff);
			}
			if (y % inc == 0)
				showProgress((double) (y - roiY) / roiHeight);
		}
	...
	/**
	 * BLUR MORE: 3x3 unweighted smoothing is implemented directly, does not convert
	 * the image to three ByteProcessors.
	 */
	private void blurMore() {
		...
		for (int y = roiY; y < roiY + roiHeight; y++) {
			...
			for (int x = roiX; x < roiX + roiWidth; x++) {
				...
				int gsum = (p1 & 0xff00) + (p2 & 0xff00) + (p3 & 0xff00) + (p4 & 0xff00) + (p5 & 0xff00) + (p6 & 0xff00)
						+ (p7 & 0xff00) + (p8 & 0xff00) + (p9 & 0xff00);
				int bsum = (p1 & 0xff) + (p2 & 0xff) + (p3 & 0xff) + (p4 & 0xff) + (p5 & 0xff) + (p6 & 0xff)
						+ (p7 & 0xff) + (p8 & 0xff) + (p9 & 0xff);
				pixels[offset++] = 0xff000000 | (((rsum + (4 << 16)) / 9) & 0xff0000)
						| (((gsum + (4 << 8)) / 9) & 0xff00) | ((bsum + 4) / 9);
			}
	 ...
	 * convertToByte() to do color conversions. The default values are 1/3, 1/3 and
	 * 1/3. Check "Weighted RGB Conversions" in <i>Edit/Options/Conversions</i> to
	 * use 0.299, 0.587 and 0.114.
	 * 
	...
	/**
	 * Returns the three weighting factors used by getPixelValue(), getHistogram()
	...
	/**
	 * Returns the values set by setRGBWeights(), or null if setRGBWeights() has not
	 * been called.
	 */
	 ...
	 *                      FloatProcessor is overwritten by this method (re-using
	 *                      its pixels array improves performance).
