...
public class GelAnalyzer implements PlugIn {
	...
	void selectFirstLane(Rectangle rect) {
		...
		/*
		 * if ( (isVertical && (rect.height/rect.width)<2 ) || (!isVertical &&
		 * (rect.width/rect.height)<2 ) ) { GenericDialog gd = new
		 * GenericDialog("Lane Orientation"); String[] orientations =
		 * {"Vertical","Horizontal"}; int defaultOrientation = isVertical?0:1;
		 * gd.addChoice("Lane Orientation:", orientations,
		 * orientations[defaultOrientation]); gd.showDialog(); if (gd.wasCanceled())
		 * return; String orientation = gd.getNextChoice();
		 * if(orientation.equals(orientations[0])) isVertical=true; else
		 * isVertical=false; }
		 */
	...
	void plotLanes(ImagePlus imp, boolean replot) {
		...
		for (int i = 1; i <= nLanes; i++) {
			if (isVertical)
				imp2.setRoi(firstRect.y, ipRotated.getHeight() - x[i] - firstRect.width, firstRect.height,
						firstRect.width);
			else
...
class PlotsCanvas extends ImageCanvas {
	...
	double[] actual = { 428566.00, 351368.00, 233977.00, 99413.00, 60057.00, 31382.00, 14531.00, 7843.00, 2146.00,
			752.00, 367.00 };
