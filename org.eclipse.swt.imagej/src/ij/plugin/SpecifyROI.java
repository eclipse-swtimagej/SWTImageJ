...
public class SpecifyROI implements PlugIn, DialogListener {
	...
	public boolean dialogItemChanged(GenericDialog gd, TypedEvent e) {
		...
		if (newWidth)
			((org.eclipse.swt.widgets.Text)(numFields.get(WIDTH))).setText(IJ.d2s(width, digits));
		if (newHeight)
			((org.eclipse.swt.widgets.Text)(numFields.get(HEIGHT))).setText(IJ.d2s(height, digits));
		digits = (scaledUnits || (int)xRoi!=xRoi || (int)yRoi!=yRoi) ? 2 : 0;
		if (newXY) {
			((org.eclipse.swt.widgets.Text)(numFields.get(X_ROI))).setText(IJ.d2s(xRoi, digits));
			((org.eclipse.swt.widgets.Text)(numFields.get(Y_ROI))).setText(IJ.d2s(yRoi, digits));
		}
