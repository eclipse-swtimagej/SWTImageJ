...
public class Compiler implements PlugIn, FilenameFilter {
	...
	void showErrors(String s) {
		if (errors == null || errors.shell.isDisposed() || !errors.shell.isVisible()) {
			errors = (Editor) IJ.runPlugIn("ij.plugin.frame.Editor", "");
			// errors.setFont(new Font("Monospaced", Font.PLAIN, errors.getFontSize()));
			//errors.setFont(new org.eclipse.swt.graphics.Font(Display.getDefault(),
					//new FontData("Monospaced", errors.getFontSize(), SWT.NORMAL)));
		}
		if (errors != null) {
