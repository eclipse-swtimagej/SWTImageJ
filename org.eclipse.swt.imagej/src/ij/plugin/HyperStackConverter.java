...
public class HyperStackConverter implements PlugIn {
	 ...
	 * @param z     slices
	 * @param t     frames
	 * @param order hyperstack order ("default", "xyctz", "xyzct", "xyztc", "xytcz"
	 *              or "xytzc")
	...
	void convertStackToHS(ImagePlus imp) {
		...
		if (IJ.recording() && Recorder.scriptMode()) {
			...
			if (order.equals(orders[0])) { // default order
				Recorder.recordCall("imp2 = HyperStackConverter.toHyperStack(imp, " + nChannels + ", " + nSlices + ", "
						+ nFrames + ", \"" + modes[mode] + "\");");
			} else {
				Recorder.recordCall("imp2 = HyperStackConverter.toHyperStack(imp, " + nChannels + ", " + nSlices + ", "
						+ nFrames + ", \"" + order + "\", \"" + modes[mode] + "\");");
			}
		}
	}
