...
public class AVI_Reader extends VirtualStack implements PlugIn {
	 ...
	 *            string, a file open dialog is shown, and the resulting ImagePlus
	 *            is displayed thereafter. The ImagePlus is not displayed if 'arg'
	 *            is a non-empty String; it can be retrieved with getImagePlus().
	 */
	 ...
	 * @return Returns the stack (may be incomplete on error); null on failure. The
	 *         stack returned may be non-null, but have a length of zero if no
	 *         suitable frames were found. Use <code>getErrorText</code> to
	 *         determine whether there has been an error reading the file. For
	 *         virtual stacks, not that I/O errors may also occur later, when
	 *         reading the frames.
	 */
	 ...
	 * it is one) where {@literal 1<=n<=nslices}. Returns null if no virtual stack
	 * or no slices or error reading the frame.
	 */
	...
	private boolean showDialog() {
		...
		gd.setInsets(2, 40, 5);
		gd.addMessage("drag and drop target", IJ.font10, ij.swt.Color.darkGray);
		...
		gd.addMessage(
				...
				IJ.font10, ij.swt.Color.darkGray);
		gd.showDialog();
		if (gd.wasCanceled())
			return false;
	...
	private long findFourccAndRead(int fourcc, boolean isList, long endPosition, boolean throwNotFoundException)
		...
		do {
			...
			if (isList && type == FOURCC_LIST)
				type = readInt();
			if (verbose)
				IJ.log("Search for '" + fourccString(fourcc) + "', found " + fourccString(type) + "' data "
	 ...
	 * behind the fourcc chunk (if successful). If not found, returns -1
	 */
	private long findFourccAndSkip(int fourcc, boolean isList, long endPosition) throws IOException {
		while (true) {
			...
			if (isList && type == FOURCC_LIST)
				type = readInt();
			if (verbose)
				IJ.log("Searching for (to skip) '" + fourccString(fourcc) + "', found " + fourccString(type) + "' data "
	...
	private void readAvi2Index(long endPosition) throws Exception, IOException {
		...
		if (bIndexType == AVI_INDEX_OF_INDEXES) { // 'indx' points to other indices
			...
			for (int i = 0; i < nEntriesInUse; i++) { // read all entries (each pointing to an ix00 index)
				...
				if (verbose)
					IJ.log("   indx entry: '" + fourccString(dwChunkId) + "' incl header "
			...
			for (int i = 0; i < nEntriesInUse; i++) {
				...
				if (frameNumber >= firstFrame && dwSize > 0) { // only valid frames (no blank frames)
					...
					if (verbose)
						IJ.log("movie data " + frameNumber + " '" + fourccString(dwChunkId) + "' "
		...
		while (true) {
			...
			if ((dwChunkId == type0xdb || dwChunkId == type0xdc) && dwSize > 0) {
				...
				if (frameNumber >= firstFrame) {
					...
					if (verbose)
						IJ.log("idx1 movie data '" + fourccString(dwChunkId) + "' " + posSizeString(framePos, dwSize)
								+ timeString());
				}
				frameNumber++;
				if (frameNumber > lastFrameToRead)
	...
	void readBitMapInfo(long endPosition) throws Exception, IOException {
		...
		if (verbose) {
			IJ.log("   > data compression=0x" + Integer.toHexString(dataCompression) + " '"
	...
	void readMovieData(long endPosition) throws Exception, IOException {
		if (verbose)
			IJ.log("MOVIE DATA " + posSizeString(endPosition - raFile.getFilePointer()) + timeString()
					+ "\nSearching for stream " + streamNumber + ": '" + fourccString(type0xdb) + "' or '"
		...
		while (true) { // loop over all chunks
			...
			if ((type == type0xdb || type == type0xdc) && size > 0) {
				IJ.showProgress((double) frameNumber / lastFrameToRead);
				if (verbose)
					IJ.log(frameNumber + " movie data '" + fourccString(type) + "' " + posSizeString(size)
							+ timeString());
				if (frameNumber >= firstFrame) {
					if (isVirtual)
	...
	/**
	 * Reads a JPEG or PNG-compressed frame from a RandomAccessFile and returns the
	 * pixels array of the resulting image and sets the ColorModel cm (if
	 * appropriate)
	 */
	...
	/**
	 * Write an intData RGB value converted from YUV, The y range between 16 and 235
	 * becomes 0...255 u, v should be between -112 and +112
	 */
	...
	final private static byte[] HUFFMAN_TABLES = new byte[] { // the 'DHT' segment
			...
			(byte) 0xC1, 0x15, 0x52, (byte) 0xD1, (byte) 0xF0, 0x24, 0x33, 0x62, 0x72, (byte) 0x82, 0x09, 0x0A, 0x16,
			...
			0x66, 0x67, 0x68, 0x69, 0x6A, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7A, (byte) 0x83, (byte) 0x84,
			...
			(byte) 0xF4, (byte) 0xF5, (byte) 0xF6, (byte) 0xF7, (byte) 0xF8, (byte) 0xF9, (byte) 0xFA, 0x11, 0x00, 0x02,
			0x01, 0x02, 0x04, 0x04, 0x03, 0x04, 0x07, 0x05, 0x04, 0x04, 0x00, 0x01, 0x02, 0x77, 0x00, 0x01, 0x02, 0x03,
			0x11, 0x04, 0x05, 0x21, 0x31, 0x06, 0x12, 0x41, 0x51, 0x07, 0x61, 0x71, 0x13, 0x22, 0x32, (byte) 0x81, 0x08,
			0x14, 0x42, (byte) 0x91, (byte) 0xA1, (byte) 0xB1, (byte) 0xC1, 0x09, 0x23, 0x33, 0x52, (byte) 0xF0, 0x15,
			0x62, 0x72, (byte) 0xD1, 0x0A, 0x16, 0x24, 0x34, (byte) 0xE1, 0x25, (byte) 0xF1, 0x17, 0x18, 0x19, 0x1A,
			0x26, 0x27, 0x28, 0x29, 0x2A, 0x35, 0x36, 0x37, 0x38, 0x39, 0x3A, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49,
			0x4A, 0x53, 0x54, 0x55, 0x56, 0x57, 0x58, 0x59, 0x5A, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6A, 0x73,
			0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7A, (byte) 0x82, (byte) 0x83, (byte) 0x84, (byte) 0x85, (byte) 0x86,
			...
			(byte) 0xDA, (byte) 0xE2, (byte) 0xE3, (byte) 0xE4, (byte) 0xE5, (byte) 0xE6, (byte) 0xE7, (byte) 0xE8,
