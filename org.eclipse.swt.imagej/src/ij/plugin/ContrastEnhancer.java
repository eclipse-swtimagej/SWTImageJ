...
public class ContrastEnhancer implements PlugIn, Measurements {
	...
	public void run(String arg) {
		...
		if(normalize) {
			...
			int bd_ = ip.getBitDepth();
			ip.setMinAndMax(0, (bd_ == 32 || bd_ == 64) ? 1.0 : ip.maxValue());
		}
		imp.updateAndDraw();
	}
	...
	void normalize(ImageProcessor ip, double min, double max) {
		...
		if(ip instanceof ShortProcessor) {
			max2 = 65535;
			range = 65536;
		} else if(ip instanceof FloatProcessor || ip instanceof ij.process.DoubleProcessor) {
			normalizeFloat(ip, min, max);
			return; // avoid running the int-LUT path on float/double processors
		}
	...
	public void equalize(ImagePlus imp) {
		...
		int bd_ = imp.getBitDepth();
		if(bd_ == 32 || bd_ == 64) {
			IJ.showMessage("Contrast Enhancer", "Equalization of " + bd_ + "-bit images not supported.");
			return;
		}
