 ...
 * commands.
 */
public class Thresholder implements PlugIn, Measurements, SelectionListener {
	...
	void convertStack(ImagePlus imp) {
		...
		if (choices != null) {
			Display.getDefault().syncExec(() -> {
				...
				((org.eclipse.swt.widgets.Combo) choices.elementAt(0)).addSelectionListener(Thresholder.this);
				((org.eclipse.swt.widgets.Combo) choices.elementAt(1)).addSelectionListener(Thresholder.this);
	...
	private void applyThreshold(ImagePlus imp, boolean oneSlice) {
		...
		ip.resetThreshold();
		if (IJ.debugMode)
			IJ.log("Thresholder (apply): " + minThreshold + "-" + maxThreshold + " " + fcolor + " " + bcolor + " "
					+ fill1 + " " + fill2);
		int[] lut = new int[256];
		for (int i = 0; i < 256; i++) {
		...
		if (fill1 && fill2 && ((fcolor == 0 && bcolor == 255) || (fcolor == 255 && bcolor == 0))) {
			...
			if (IJ.debugMode)
				IJ.log("Thresholder: " + fcolor + "-" + fcolor + " (" + (Prefs.blackBackground ? "black" : "white")
						+ " background)");
		}
	...
	public void widgetSelected(SelectionEvent e) {
		...
		if (choice == choices.elementAt(0))
			method = choice.getItem(choice.getSelectionIndex());
		else
			background = choice.getItem(choice.getSelectionIndex());
		updateThreshold(imp);
	}
