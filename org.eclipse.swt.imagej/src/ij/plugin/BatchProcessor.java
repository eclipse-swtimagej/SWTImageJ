 ...
 * commands.
 */
public class BatchProcessor implements PlugIn, Runnable {
	...
	private static final String[] code = { "[Select from list]", "Add Border", "Convert to RGB", "Crop",
			"Gaussian Blur", "Invert", "Label", "Timestamp", "Max Dimension", "Measure", "Print Index and Title",
			"Resize", "Scale", "Show File Info", "Unsharp Mask", };
	private static final String help = "<html>" + "<h1>Process&gt;Batch&gt;Virtual Stack</h1>" + "<font size=+1>"
			...
			+ "before clicking on <i>Process</i>.<br>" + "<br>"
			...
			+ "effectively removing it from the output virtual stack.<br> <br>" + "</font>";
	private String macro = "";
	private int testImage;
	private org.eclipse.swt.widgets.Button input, output, open, save, test;
	Text inputDir;
	...
	boolean showDialog() {
		...
		org.eclipse.swt.widgets.Combo choice = (org.eclipse.swt.widgets.Combo) choices.elementAt(1);
		if (virtualStack != null)
			gd.addHelp(help);
	...
	void addPanels(GenericDialog gd) {
		...
		org.eclipse.swt.layout.GridLayout rl_composite = new org.eclipse.swt.layout.GridLayout(2, true);
		p.setLayout(rl_composite);
		p.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false, 1, 1));
		if (virtualStack == null) {
			input = new org.eclipse.swt.widgets.Button(p, SWT.NONE);
			input.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false, 1, 1));
			input.setText("Input...");
			...
			// p.add(input);
			inputDir = new org.eclipse.swt.widgets.Text(p, SWT.BORDER);
		...
		output.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false, 1, 1));
		...
		// p.add(output);
		outputDir = new org.eclipse.swt.widgets.Text(p, SWT.BORDER);
	...
	void addButtons(GenericDialog gd) {
		...
		// Panel p = new Panel();
		Composite p = new Composite(gd.getShell(), SWT.NONE);
		org.eclipse.swt.layout.GridLayout rl_composite = new org.eclipse.swt.layout.GridLayout(3, true);
		p.setLayout(rl_composite);
		p.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
		// p.setLayout(new FlowLayout(FlowLayout.CENTER, 5, 0));
		test = new org.eclipse.swt.widgets.Button(p, SWT.NONE);
		test.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
		test.setText("Test");
		...
		// p.add(test);
		open = new org.eclipse.swt.widgets.Button(p, SWT.NONE);
		open.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
		open.setText("Open...");
		...
		// p.add(open);
		save = new org.eclipse.swt.widgets.Button(p, SWT.NONE);
		save.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
		save.setText("Save...");
	...
	public void itemStateChanged(SelectionEvent e) {
		...
		if (code != null) {
			Text ta = gd.getTextArea1();
			ta.setSelection(ta.getCaretPosition());
			ta.insert(code);
			if (IJ.isMacOSX())
				ta.setFocus();
		}
	...
	/*
	 * Changed for the SWT implementation because we load at least when developing
	 * not from a *.jar file! So we add an extra condition here!
	 */
	public static String openMacroFromJar(String name) {
		...
		try {
			...
			if (is != null) {
				...
				String path = IJ.getDirectory("macros");
				File fileMacro = new File(path + name);
				InputStream inStream = new FileInputStream(fileMacro);
	...
	public void run() {
		...
		Display.getDefault().syncExec(() -> {
			...
			org.eclipse.swt.graphics.Point loc = new org.eclipse.swt.graphics.Point(10, 30);
			if (testImage != 0) {
