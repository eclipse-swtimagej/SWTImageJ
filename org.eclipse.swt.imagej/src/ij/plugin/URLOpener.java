...
public class URLOpener implements PlugIn {
	...
	public void run(String urlOrName) {
		if(IJ.debugMode)
			IJ.log("URLOpener.run: " + urlOrName);
		if(!urlOrName.equals("")) {
		...
		gd.addMessage("Enter URL of an image, macro or web page", null, ij.swt.Color.darkGray);
	...
	public static String[] getSampleImageNames() {
		...
		org.eclipse.swt.widgets.Menu samplesMenu = Menus.getImageJMenu("File>Open Samples");
		if(samplesMenu == null)
			return new String[0];
		for(int i = 0; i < samplesMenu.getItemCount(); i++) {
			org.eclipse.swt.widgets.MenuItem menuItem = samplesMenu.getItem(i);
			if(menuItem.getListeners(SWT.Selection).length == 0)
				continue; // separator?
