...
public class PointToolOptions implements PlugIn, DialogListener {
	...
	public static final String help = "<html>" + "<h1>Point Tool</h1>" + "<font size=+1>" + "<ul>"
	...
	public static void update() {
		if (gd != null && gd.isVisible()) {
			...
			if (choices == null || choices.size() < 4)
				return;
			org.eclipse.swt.widgets.Combo counterChoice = (org.eclipse.swt.widgets.Combo) choices.elementAt(3);
			int counter = getCounter();
			int count = getCount(counter);
			counterChoice.select(counter);
			((org.eclipse.swt.widgets.Label) gd.getMessage()).setText("" + count);
		}
	}
