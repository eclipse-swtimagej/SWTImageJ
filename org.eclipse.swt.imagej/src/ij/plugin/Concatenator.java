 ...
 */
public class Concatenator implements PlugIn, SelectionListener {
	...
	public ImagePlus concatenateHyperstacks(ImagePlus[] images, String newTitle, boolean keep) {
		...
		for (int i = 1; i < n; i++) {
			if (images[i].getNFrames() > 1)
				concatSlices = false;
			if (images[i].getBitDepth() != bitDepth || images[i].getNChannels() != channels
			...
			if (cal2.pixelWidth != cal.pixelWidth || cal2.pixelHeight != cal.pixelHeight
	...
	public void widgetSelected(SelectionEvent e) {
		...
		Event event = new org.eclipse.swt.widgets.Event();
		event.type = SWT.SELECTED;
	...
	public void itemStateChanged(Event ie) {
		org.eclipse.swt.widgets.Combo c;
		if (ie.widget == allWindows) { // User selected / unselected 'all windows' button
			int count = 0;
			if (allWindows.getSelection()) {
				for (Enumeration e = choices.elements(); e.hasMoreElements();) {
					c = (org.eclipse.swt.widgets.Combo) e.nextElement();
					c.select(count++);
					c.setEnabled(false);
				}
			} else {
			...
			for (Enumeration e = choices.elements(); e.hasMoreElements();) {
				...
				if (!foundNone) {
					c.setEnabled(true);
					if (c.getItem(c.getSelectionIndex()).equals(none))
						foundNone = true;
				} else { // a previous choice was 'none'
					for (int i = 0; i < c.getItemCount(); i++) {
						String name = c.getItem(i);
						if (name.equals(none)) {
							c.select(i);
							c.setEnabled(false);
						}
