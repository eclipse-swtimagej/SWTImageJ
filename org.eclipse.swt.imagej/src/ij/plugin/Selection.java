...
public class Selection implements PlugIn, Measurements {
	 ...
	 * centroid, otherwise use<br> the Pratt method to fit a circle to the points
	 * that define the line or multi-point selection.<br> Reference: Pratt V.,
	 * Direct least-squares fitting of algebraic surfaces", Computer Graphics, Vol.
	 * 21, pages 145-152 (1987).<br> Original code: Nikolai Chernov's MATLAB script
	 * for Newton-based Pratt fit.<br>
	 * (http://www.math.uab.edu/~chernov/cl/MATLABcircle.html)<br> Java version:
	 * https://github.com/mdoube/BoneJ/blob/master/src/org/doube/geometry/FitCircle.
	 * java<br> Authors: Nikolai Chernov, Michael Doube, Ved Sharma
	 */
	 ...
	 * imp) { int smallestY = imp.getHeight(); int x, y; for (int i=0; i<n; i++) { y
	 * = yCoordinates[i]; if (y<smallestY) smallestY = y; } int smallestX =
	 * imp.getWidth(); int p1 = 0; for (int i=0; i<n; i++) { x = xCoordinates[i]; y
	 * = yCoordinates[i]; if (y==smallestY && x<smallestX) { smallestX = x; p1 = i;
	 * } } return p1; }
	 */
