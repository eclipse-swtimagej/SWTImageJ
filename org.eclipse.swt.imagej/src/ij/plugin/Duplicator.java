 ...
 * </pre>
 */
public class Duplicator implements PlugIn, SelectionListener, VerifyListener {
	...
	private org.eclipse.swt.widgets.Button stackCheckbox;
	private org.eclipse.swt.widgets.Text titleField, rangeField;
	private org.eclipse.swt.widgets.Text[] rangeFields;
	...
	String showDialog(ImagePlus imp, String dialogTitle, String prompt) {
		...
		if(stackSize > 1) {
			...
			if(!isMacro) {
				stackCheckbox = (org.eclipse.swt.widgets.Button)(gd.getCheckboxes().elementAt(gd.getCheckboxes().size() - 1));
	...
	private String getNewTitle() {
		...
		if(imp.getStackSize() > 1 && !duplicateStack && !legacyMacro && (stackCheckbox == null || !stackCheckbox.getSelection())) {
			ImageStack stack = imp.getStack();
	...
	/*
	 * Returns the part of 'roi' overlaping 'imp' Author Marcel Boeglin 2013.12.15
	 */
	...
	public void textValueChanged(VerifyEvent e) {
		...
		if(e.getSource() == titleField) {
			...
			stackCheckbox.setSelection(true);
	}

	public void itemStateChanged(SelectionEvent e) {

		duplicateStack = stackCheckbox.getSelection();
		if(titleField != null) {
			String title = getNewTitle();
