...
public class Resizer implements PlugIn, ModifyListener, SelectionListener {
	...
	public void run(String arg) {
		...
		if(crop) {
			...
			if(!IJ.macroRunning()) {
				Display.getDefault().syncExec(() -> {
					for(int i = 0; i < 2; i++)
						((org.eclipse.swt.widgets.Text)fields.elementAt(i)).addModifyListener(Resizer.this);
				});
			}
			checkboxes = gd.getCheckboxes();
			if(!IJ.macroRunning())
				Display.getDefault().syncExec(() -> {
					((org.eclipse.swt.widgets.Button)checkboxes.elementAt(0)).addSelectionListener(Resizer.this);
				});
			gd.showDialog();
			if(gd.wasCanceled()) {
				imp.unlock();
			...
			if(options != null && options.contains("constrain") && options.contains("height=") && !options.contains("width="))
				sizeToHeight = true;
		...
		if(imp2 != null && imp2 != imp) {
			...
			imp.close();
			imp2.show();
		} else if(crop && (bitDepth == 16 || bitDepth == 32 || bitDepth == 64)) {
	...
	public void itemStateChanged(SelectionEvent e) {
		...
		org.eclipse.swt.widgets.Button cb = (org.eclipse.swt.widgets.Button)checkboxes.elementAt(0);
		boolean newConstrain = cb.getSelection();
