...
public class Analyzer implements PlugInFilter, Measurements {
	...
	private static final int[] list = {AREA, MEAN, STD_DEV, MODE, MIN_MAX, CENTROID, CENTER_OF_MASS, PERIMETER, RECT, ELLIPSE, SHAPE_DESCRIPTORS, FERET, INTEGRATED_DENSITY, MEDIAN, SKEWNESS, KURTOSIS, AREA_FRACTION, STACK_POSITION, LIMIT, LABELS, INVERT_Y, SCIENTIFIC_NOTATION, ADD_TO_OVERLAY, NaN_EMPTY_CELLS};
	...
	public void saveResults(ImageStatistics stats, Roi roi) {
		...
		if((measurements & FERET) != 0) {
			boolean extras = true;
			double FeretDiameter = Double.NaN, feretAngle = Double.NaN,
					minFeret = Double.NaN, feretX = Double.NaN,
					feretY = Double.NaN;
			Roi roi2 = roi;
	...
	void savePoints(PointRoi roi) {
		...
		// Read the sampled intensity at full precision for 64-bit (double) images;
		// ip.getPixelValue(...) returns a float and would narrow the value.
		double value;
		if(imp.getBitDepth() == 64 && ip instanceof ij.process.DoubleProcessor)
			value = ((ij.process.DoubleProcessor)ip).getPixelValueDouble(ix, iy);
		else
			value = ip.getPixelValue(ix, iy);
	...
	public synchronized static boolean resetCounter() {
		...
		if(counter > 0 && lineCount > 0 && unsavedMeasurements && !macro && ij != null && !ij.quitting()) {
			YesNoCancelDialog d = new YesNoCancelDialog("ImageJ", "Save " + counter + " measurements?");
			if(d.cancelPressed())
				return false;
