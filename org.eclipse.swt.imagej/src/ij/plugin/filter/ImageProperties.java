...
public class ImageProperties implements PlugInFilter, org.eclipse.swt.events.ModifyListener {

	private final String SAME = "-";
	ImagePlus imp;
	static final int NANOMETER = 0, MICROMETER = 1, MILLIMETER = 2,
			CENTIMETER = 3, METER = 4, KILOMETER = 5, INCH = 6, FOOT = 7,
			MILE = 8, PIXEL = 9, OTHER_UNIT = 10;
	int oldUnitIndex;
	...
	org.eclipse.swt.widgets.Text pixelWidthField, pixelHeightField,
			pixelDepthField;
	...
	public int setup(String arg, ImagePlus imp) {
		...
		this.imp = imp;
		// DOES_ALL covers the legacy 5 (8G, 8C, 16, 32, RGB) but not DOES_64.
		// Image > Properties... must accept 64-bit DoubleProcessor images so
		// the user can inspect/edit calibration, dimensions, and frame
		// interval. The dialog itself is bit-depth-agnostic — it only edits
		// FileInfo metadata (Calibration, dimensions, units), never pixels.
	...
	void showDialog(ImagePlus imp) {
		...
		gd.setInsets(0, 5, 0);
		gd.addMessage("Note: c*z*t must equal " + stackSize, null, ij.swt.Color.darkGray);
		gd.setInsets(15, 0, 0);
		if(legacyMacro)
		...
		boolean reset = false;
		boolean xUnitChanged = false, yUnitChanged = false,
				zUnitChanged = false;
		if(legacyMacro) {
	...
	public void textValueChanged(ModifyEvent e) {
		...
		int channels = (int)Tools.parseDouble(((org.eclipse.swt.widgets.Text)nfields.elementAt(2)).getText(), -99);
		int depth = (int)Tools.parseDouble(((org.eclipse.swt.widgets.Text)nfields.elementAt(3)).getText(), -99);
		int frames = (int)Tools.parseDouble(((org.eclipse.swt.widgets.Text)nfields.elementAt(4)).getText(), -99);
		...
		org.eclipse.swt.widgets.Text unitField = (org.eclipse.swt.widgets.Text)sfields.elementAt(0);
