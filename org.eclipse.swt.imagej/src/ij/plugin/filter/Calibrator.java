...
public class Calibrator implements PlugInFilter, Measurements, ActionListener {
	...
	org.eclipse.swt.widgets.Button open;
	private org.eclipse.swt.widgets.Button save;
	...
	public int setup(String arg, ImagePlus imp) {
		...
		this.imp = imp;
		// Include DOES_64 so 64-bit DoubleProcessor images can invoke this
		// command to set/change the value unit label. Calibration functions
		// (curve fits) are not applicable to 64-bit float pixel data; the
		// run() method restricts 64-bit images to unit-label edits only,
		// matching the existing behaviour for 32-bit float images.
		return DOES_ALL - DOES_RGB + DOES_64 + NO_CHANGES;
	}
	...
	public void run(ImageProcessor ip) {
		...
		if(choiceIndex == customIndex) {
			...
			return;
		} else if(imp.getType() == ImagePlus.GRAY32 || imp.getType() == ImagePlus.GRAY64) {
			if(choiceIndex == 0)
				imp.getCalibration().setValueUnit(unit);
			else {
				String bitDepth = imp.getType() == ImagePlus.GRAY64 ? "64-bit" : "32-bit";
				IJ.error("Calibrate", "Function must be \"None\" for " + bitDepth + " images,\nbut you can change the Unit.");
			}
		} else
			calibrate(imp);
	}
	...
	Composite makeButtonPanel(GenericDialog gd) {
		...
		buttons.setLayout(rl_composite);
		buttons.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
		// buttons.setLayout(new FlowLayout(FlowLayout.CENTER, 5, 0));
		/*
		 * open = new Button("Open..."); open.addActionListener(this);
		 * buttons.add(open); save = new Button("Save...");
		 * save.addActionListener(this); buttons.add(save);
		 */
		open = new org.eclipse.swt.widgets.Button(buttons, SWT.NONE);
		open.setText("Open...");
		;
		...
		save = new org.eclipse.swt.widgets.Button(buttons, SWT.NONE);
		save.setText("Save...");
	...
	void save() {
		...
		Text ta1 = gd.getTextArea1();
		Text ta2 = gd.getTextArea2();
		ta1.selectAll();
		String text1 = ta1.getText();
		ta1.setSelection(0, 0);
		ta2.selectAll();
		String text2 = ta2.getText();
		ta2.setSelection(0, 0);
