...
public class MaximumFinder implements ExtendedPlugInFilter, DialogListener {
	...
	private boolean thresholdWarningShown = false; // whether the warning "can't find minima with thresholding" has been
													// shown
	private org.eclipse.swt.widgets.Label messageArea; // reference to the textmessage area for displaying the number of
														// maxima
	...
	public int showDialog(ImagePlus imp, String command, PlugInFilterRunner pfr) {
		...
		messageArea = (org.eclipse.swt.widgets.Label)gd.getMessage();
	 ...
	 * the number of maxima. There is an example at<br>
	 * http://imagej.net/ij/macros/js/FindMaxima.js.
	 * 
	 * @param ip
	 *            The input image
	 * @param tolerance
	 *            Height tolerance: maxima are accepted only if
	 *            protruding more than this value from the ridge to a
	 *            higher maximum
	 * @param excludeOnEdges
	 *            Whether to exclude edge maxima. Also determines whether
	 *            strict mode is on, i.e., whether the global maximum is
	 *            accepted even if all other pixel are less than
	 *            'tolerance' below this level (In 1.52m and before,
	 *            'strict' and 'excludeOnEdges' were the same).
	 * @return A Polygon containing the coordinates of the maxima, where
	 *         poly.npoints is the number of maxima. Note that poly.xpoints.length
	 *         may be greater than the number of maxima.
	 */
	 ...
	 *            higher maximum
	 * @param strict
	 *            When off, the global maximum is accepted even if all
	 *            other pixel are less than 'tolerance' below this level.
	 *            With <code>excludeOnEdges=true</code>, 'strict' also
	 *            means that the surounding of a maximum within
	 *            'tolerance' must not include an edge pixel (otherwise,
	 *            it is enough that there is no edge pixel with the
	 *            maximum value).
	 * @param excludeOnEdges
	 *            Whether to exclude edge maxima. Also determines whether
	 *            strict mode is on, i.e., whether the global maximum is
	...
	public Polygon getMaxima(ImageProcessor ip, double tolerance, boolean strict, boolean excludeOnEdges) {
		...
		findMaxima(ip, tolerance, strict, ImageProcessor.NO_THRESHOLD, MaximumFinder.POINT_SELECTION, excludeOnEdges, false);
	 ...
	 * @param tolerance
	 *            Depth of a qualified valley must exceed tolerance. Tolerance
	 *            must be >= 0. Flat tops are marked at their centers.
	 * @param edgeMode
	 *            0=include, 1=exclude, 2=circular edgeMode = 0 (include
	 *            edges) peak may be separated by one qualified valley and by
	 *            a border. edgeMode = 1 (exclude edges) peak must be
	 *            separated by two qualified valleys edgeMode = 2 (circular)
	 *            array is regarded to be circular
	 ...
	 *            higher maximum
	 * @param outputType
	 *            What to mark in output image: SINGLE_POINTS,
	 *            IN_TOLERANCE or SEGMENTED. No output image is created
	 *            for output types POINT_SELECTION, LIST and COUNT.
	 * @param excludeOnEdges
	 *            Whether to exclude edge maxima. Also determines whether
	 *            strict mode is on, i.e., whether the global maximum is
	 *            accepted even if all other pixel are less than
	 ...
	 *         the input ip are not set. Returns null if outputType does not require
	 *         an output or if cancelled by escape
	 */
	 ...
	 * segmentation lines may be improperly placed if local maxima are suppressed by
	 * the tolerance.
	 *
	 * @param ip
	 *            The input image
	 ...
	 *            higher maximum
	 * @param threshold
	 *            minimum height of a maximum (uncalibrated); for no
	...
	public ByteProcessor findMaxima(ImageProcessor ip, double tolerance, boolean strict, double threshold, int outputType, boolean excludeOnEdges, boolean isEDM) {
		...
		long[] maxPoints = maximumPossible ? getSortedMaxPoints(ip, typeP, excludeEdgesNow, isEDM, globalMin, globalMax, threshold) : new long[0];
		if(Thread.currentThread().isInterrupted())
			return null;
		IJ.showStatus("Analyzing  maxima...");
		float maxSortingError = 0;
		if(ip instanceof FloatProcessor || ip instanceof ij.process.DoubleProcessor) // sorted sequence may be inaccurate by this value
	 ...
	 * @param typeP
	 *            A byte image, same size as ip, where the maximum
	 *            points are marked as MAXIMUM (do not use it as output:
	 *            for rois, the points are shifted w.r.t. the input
	 *            image)
	 ...
	 * @param threshold
	 *            The threshold (calibrated) below which no pixels are
	 ...
	 *         points marked as MAXIMUM in typeP, they are invalid for images with a
	 *         roi.
	 */
	long[] getSortedMaxPoints(ImageProcessor ip, ByteProcessor typeP, boolean excludeEdgesNow, boolean isEDM, float globalMin, float globalMax, double threshold) {
		...
		for(int y = roi.y; y < roi.y + roi.height; y++) { // find local maxima now
			if(y % 50 == 0 && thread.isInterrupted())
				return null;
			for(int x = roi.x,
					i = x + y * width; x < roi.x + roi.width; x++, i++) { // for better performance with
		...
		for(int y = roi.y; y < roi.y + roi.height; y++) // enter all maxima into an array
			for(int x = roi.x,
					p = x + y * width; x < roi.x + roi.width; x++, p++)
				if(types[p] == MAXIMUM) {
	 ...
	 *            the image to be analyzed
	 * @param typeP
	 *            8-bit image, here the point types are marked by type:
	 *            MAX_POINT, etc.
	 * @param maxPoints
	 *            input: a list of all local maxima, sorted by height.
	 ...
	 *            sorting may be inaccurate, sequence may be reversed
	 *            for maxima having values not deviating from each other
	 *            by more than this (this could be a result of precision
	 *            loss when sorting ints instead of floats, or because
	 *            sorting does not take the height correction in
	 *            'trueEdmHeight' into account
	 * @param outputType
	 */
	void analyzeAndMarkMaxima(ImageProcessor ip, ByteProcessor typeP, long[] maxPoints, boolean excludeEdgesNow, boolean isEDM, float globalMin, double tolerance, boolean strict, int outputType, float maxSortingError) {
	 ...
	 * @param threshold
	 *            Pixels of ip below this value (calibrated) are considered
	 ...
	 * marked maximum is touched (or the plateau of a previously eliminated maximum
	 * leads to a marked maximum). Then set all the points above this value to this
	 * value
	 * 
	 * @param outIp
	 ...
	 * @param maxPoints
	 *            array containing the coordinates of all maxima that might be
	 *            relevant
	 */
	 ...
	 * 
	 * @param ip
	 *            8-bit image with background = 0, lines between 1 and 254 and
	 ...
	 * 
	 * @param typeP
	 *            Here the pixel types of the original image are noted, pixels
	 *            with bit MAX_AREA at the edge are considered indicators of an
	 *            edge maximum.
	 * @param ip
	 *            the image resulting from watershed segmentaiton (foreground
	...
	/**
	 * Do watershed segmentation on a byte image, with the start points (maxima) set
	 * to 255 and the background set to 0. The image should not have any local
	 ...
	 * 
	 * @param ip
	 *            The byteProcessor containing the image, with size given by the
		...
		for(int level = highestValue; level >= 1; level--) {
			...
			if(remaining > 0 && level > 1) { // any pixels that we have not reached?
				...
				if(nextLevel > 0) {
					...
					for(int i = 0,
							p = levelStart[level]; i < remaining; i++, p++) {
					...
					// IJ.log("level="+level+": add "+(newNextLevelEnd-levelStart[nextLevel+1])+"
					// points to "+nextLevel);
	 ...
	 * @param ip
	 *            the EDM with the segmeted blobs successively getting
	 *            set to 255
	 * @param table
	 *            The fateTable
	 * @param levelStart
	 ...
	 * @param pixelPointers[]
	 *            list of pixel coordinates (x+y*width) sorted by level
	 *            (in sequence of y, x within each level)
	 * @param xCoordinates
	 *            list of x Coorinates for the current level only (no
	 *            offset levelStart)
	 ...
	 * neighborhood: x-1 x x+1 y-1 128 1 2 y 64 pxl_unset_yet 4 y+1 32 16 8 (to find
	 * throws entry, sum up the numbers of the neighboring pixels set; e.g. entry
	 * 6=2+4 if only the pixels (x,y-1) and (x+1, y-1) are set. A pixel is added on
	 * the 1st pass if bit 0 (2^0 = 1) is set, on the 2nd pass if bit 1 (2^1 = 2) is
	 * set, etc. pass gives the direction of rotation, with 0 = to top left
	 * (x--,y--), 1 to top, and clockwise up to 7 = to the left (x--). E.g. 4 = add
	 * on 3rd pass, 3 = add on either 1st or 2nd pass.
	 */
	 ...
	 * coordinates in single ints for watershed: intEncodeXMask, intEncodeYMask and
	 * intEncodeShift. E.g., for width between 129 and 256, xMask=0xff and yMask =
	 * 0xffffff00 are bitwise masks for x and y, respectively, and shift=8 is the
	 * bit shift to get y from the y-masked value Returns as class variables: the
	 * arrays of the offsets to the 8 neighboring pixels and the array maskAndShift
	 * for watershed
	 */
	 ...
	 * 
	 * @param x
	 *            x-coordinate of the pixel that has a neighbor in the given
	 *            direction
	 * @param y
	 *            y-coordinate of the pixel that has a neighbor in the given
	 *            direction
	 * @param direction
	 *            the direction from the pixel towards the neighbor (see
	 *            makeDirectionOffsets)
