...
public class MaximumFinder implements ExtendedPlugInFilter, DialogListener {
	...
	public boolean dialogItemChanged(GenericDialog gd, TypedEvent e) {
		...
		if (useMinThreshold && ((invertedLut && !lightBackground) || (!invertedLut && lightBackground))) {
			if (!thresholdWarningShown)
				if (!IJ.showMessageWithCancel("Find Maxima",
	...
	public Polygon getMaxima(ImageProcessor ip, double tolerance, boolean strict, boolean excludeOnEdges) {
		findMaxima(ip, tolerance, strict, ImageProcessor.NO_THRESHOLD, MaximumFinder.POINT_SELECTION, excludeOnEdges,
				false);
	 ...
	 * @param tolerance Depth of a qualified valley must exceed tolerance. Tolerance
	 *                  must be >= 0. Flat tops are marked at their centers.
	 * @param edgeMode  0=include, 1=exclude, 2=circular edgeMode = 0 (include
	 ...
	 *         an output or if cancelled by escape
	 */
	public ByteProcessor findMaxima(ImageProcessor ip, double tolerance, double threshold, int outputType,
			boolean excludeOnEdges, boolean isEDM) {
	...
	public ByteProcessor findMaxima(ImageProcessor ip, double tolerance, boolean strict, double threshold,
		...
		if (threshold != ImageProcessor.NO_THRESHOLD && ip.getCalibrationTable() != null && threshold > 0
	 ...
	 *         roi.
	 */
	long[] getSortedMaxPoints(ImageProcessor ip, ByteProcessor typeP, boolean excludeEdgesNow, boolean isEDM,
		...
		for (int level = highestValue; level >= 1; level--) {
			...
			while (remaining > 0 && idle < 8) {
				...
				do { // expand each level in 8 directions
					int n = processLevel(directionSequence[dIndex % 8], ip, table, levelStart[level], remaining,
							coordinates, setPointList);
	 ...
	 */
	private int processLevel(int pass, ImageProcessor ip, int[] fateTable, int levelStart, int levelNPoints,
