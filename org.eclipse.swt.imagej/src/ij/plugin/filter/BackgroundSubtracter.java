...
public class BackgroundSubtracter implements ExtendedPlugInFilter, DialogListener {
	...
	private final static int X_DIRECTION = 0, Y_DIRECTION = 1, DIAGONAL_1A = 2,
			DIAGONAL_1B = 3, DIAGONAL_2A = 4, DIAGONAL_2B = 5; // filter directions
	 ...
	 * 
	 * @param fp
	 *            Input image, will be modified by setting the pixel values to the background
	 *            resulting from the operation
	 * @param direction
	 *            Determines for lines in which direction the parabola background is applied.
	 * @param coeff2
	 *            Magnitude of the 2nd order coefficient of the polynomial describing the parabola.
	 * @param nextPoint
	 *            Work array.
	 */
	...
	static float[] lineSlideParabola(float[] pixels, int start, int inc, int length, float coeff2, float[] cache, int[] nextPoint, float[] correctedEdges) {
		...
		for(int i = 0, p = start; i < length; i++, p += inc) {
			...
			if(v < minValue)
				minValue = v;
			if(i >= 2 && (vPrevious1 + vPrevious1 - vPrevious2 - v < curvatureTest || ((Float.isNaN(vPrevious2) || Float.isNaN(v)) && !Float.isNaN(vPrevious1)))) {
		...
		while(i1 < length - 1) {
			...
			if(i2 <= i1)
				i2 = length - 1; // no 2nd point found (e.g. all NaN)
		...
		if(correctedEdges != null) {
			if(firstCorner > lastCorner) {
				...
				float slope = lastCorner - firstCorner > length / 4 + 1 ? (v2 - v1) / (lastCorner - firstCorner) : // slope of the line through the two outermost non-edge touching points
	...
	FloatProcessor shrinkImage(FloatProcessor ip, int shrinkFactor) {
		...
		for(int ySmall = 0; ySmall < sHeight; ySmall++) {
			for(int xSmall = 0; xSmall < sWidth; xSmall++) {
				min = Float.MAX_VALUE;
				for(int j = 0,
						y = shrinkFactor * ySmall; j < shrinkFactor && y < height; j++, y++) {
					for(int k = 0,
							x = shrinkFactor * xSmall; k < shrinkFactor && x < width; k++, x++) {
		...
		for(int y = -radius; y < height + radius; y++) { // for all positions of the ball center:
			...
			for(int x = -radius; x < width + radius; x++) {
				...
				for(int yp = y0, yBall = yBall0; yp <= yend; yp++, yBall++) { // for all points inside the ball
					...
					for(int xp = x0,
							bp = xBall0 + yBall * ballWidth; xp <= xend; xp++, cachePointer++, bp++) {
				...
				for(int yp = y0, yBall = yBall0; yp <= yend; yp++, yBall++) // raise pixels to ball surface
					for(int xp = x0, p = xp + yp * width,
							bp = xBall0 + yBall * ballWidth; xp <= xend; xp++, p++, bp++) {
		...
		for(int y = 0; y < height; y++) {
			if(ySmallLine0 < ySmallIndices[y]) {
				...
				for(int x = 0; x < width; x++) // x-interpolation of the new smallImage line -> line1
					line1[x] = sPixels[sYPointer + xSmallIndices[x]] * xWeights[x] + sPixels[sYPointer + xSmallIndices[x] + 1] * (1f - xWeights[x]);
			}
...
class RollingBall {
	...
	void buildRollingBall(double ballradius, int arcTrimPer) {
		...
		// this.shrinkFactor = shrinkFactor; // TODO
