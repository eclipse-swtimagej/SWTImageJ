...
public class GaussianBlur implements ExtendedPlugInFilter, DialogListener {
	...
	private int flags = DOES_ALL | DOES_64 | SUPPORTS_MASKING | KEEP_PREVIEW;
	 ...
	 * @param sigma
	 *            Standard deviation of the Gaussian (pixels)
	 *
	...
	public void blur1Direction(final FloatProcessor ip, final double sigma, final double accuracy, final boolean xDirection, final int extraLines) {
		...
		int numThreads2 = (int)((lineTo - lineFrom) * (long)(writeTo - writeFrom) * gaussKernel[0].length / (doDownscaling ? 8000 : 16000)) + 1; // use fewer threads if a small task
	 ...
	 */
	final static private void downscaleLine(final float[] pixels, final float[] cache, final float[] kernel, final int reduceBy, final int pixel0, final int unscaled0, final int length, final int pointInc, final int newLength) {
	 ...
	 */
	final static private void upscaleLine(final float[] cache, final float[] pixels, final float[] kernel, final int reduceBy, final int pixel0, final int unscaled0, final int writeFrom, final int writeTo, final int pointInc) {
	 ...
	 */
	final static private void convolveLine(final float[] input, final float[] pixels, final float[][] kernel, final int readFrom, final int readTo, final int writeFrom, final int writeTo, final int point0, final int pointInc) {
	...
	public static void resetOutOfRoi(ImageProcessor ip, int radius) {
		...
		for(int y = roi.y + roi.height,
				p = width * y + roi.x; y < yEnd; y++, p += width)
			System.arraycopy(snapshot, p, pixels, p, roi.width);
	}
