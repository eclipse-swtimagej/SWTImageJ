...
public class ThresholdToSelection implements PlugInFilter {
				...
				if(thisRow[x + 1]) { // i.e., pixel (x,y) is selected
					if(!prevRow[x + 1]) {
						...
						if(outline[x] == null) {
							...
							if(x < w - 1 && y < h && x != xAfterLowerRightCorner && !thisRow[x + 2] && prevRow[x + 2]) { // at lower right corner & next pxl deselected
								outline[x] = null;
					...
					if(prevRow[x + 1]) {
						...
						if(outline[x] == null) {
							...
							if(x < w - 1 && y < h && x != xAfterLowerRightCorner && thisRow[x + 2] && !prevRow[x + 2]) { // at lower right corner & next pxl selected
								outline[x] = null;
								// outline[x+1] unchanged
								outline[x + 1].append(x + 1, y);
								xAfterLowerRightCorner = x + 1;
	...
	public int setup(String arg, ImagePlus imp) {
		...
		image = imp;
		return DOES_8G | DOES_16 | DOES_32 | DOES_64 | NO_CHANGES;
	}
