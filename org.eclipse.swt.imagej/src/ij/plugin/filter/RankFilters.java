...
public class RankFilters implements ExtendedPlugInFilter, DialogListener {
	...
	public static final int MEAN = 0, MIN = 1, MAX = 2, VARIANCE = 3,
			MEDIAN = 4, OUTLIERS = 5, DESPECKLE = 6, REMOVE_NAN = 7, OPEN = 8,
			CLOSE = 9, TOP_HAT = 10; // when adding a new filter, set HIGHEST_FILTER below.
	 ...
	 * @param whichOutliers
	 *            BRIGHT_OUTLIERS or DARK_OUTLIERS for 'outliers' filter
	 * @param threshold
	 ...
	 *            for top-hat background subtraction, background is light, not dark
	 * @param dontSubtract
	 *            fpr top-hat filter, performs a grayscale open or close instead of top-hat,
	...
	public void rank(ImageProcessor ip, double radius, int filterType, int whichOutliers, float threshold, boolean lightBackground, boolean dontSubtract) {
		...
		boolean snapshotRequired = (filterType == TOP_HAT && !dontSubtract) || (isMultiStepFilter(filterType) && (roi.width != ip.getWidth() || roi.height != ip.getHeight()));
		...
		for(int ch = 0; ch < ip.getNChannels(); ch++) {
			...
			if(filterType == TOP_HAT && !dontSubtract) { // top-hat filter: Subtract opened from input
				...
				for(int y = roi.y; y < roi.y + roi.height; y++)
					for(int ix = 0,
							p = y * width + roi.x; ix < roi.width; ix++, p++)
		...
		for(int i = 0; i < numThreads - 1; i++) {
			...
			callables[i] = new Callable<Void>() {
				...
				final public Void call() {
					...
					doFiltering(ip, lineRadii, cache, cacheWidth, cacheHeight, filterType, minMaxOutliersSign, threshold, colorChannel, yForThread, threadNumber, nextY);
					return null;
				}
			};
		}
		Future[] futures = ThreadUtil.start(callables);
		doFiltering(ip, lineRadii, cache, cacheWidth, cacheHeight, filterType, minMaxOutliersSign, threshold, colorChannel, yForThread, /* threadNumber= */0, nextY);
		...
		while(true) {
			...
			if(numThreads == 1) { // R E A D
				...
				for(int yNew = yStartReading; yNew <= y + kHeight / 2; yNew++) { // only 1 line except at start
					readLineToCacheOrPad(pixels, width, height, roi.y, xminInside, widthInside, cache, cacheWidth, cacheHeight, padLeft, padRight, colorChannel, kHeight, yNew);
				}
			...
			filterLine(values, width, cache, cachePointers, kNPoints, cacheLineP, roi, y, // F I L T E R
					sums, medianBuf1, medianBuf2, minMaxOutliersSign, maxValue, isFloat, filterType, smallKernel, sumFilter, minOrMax, minOrMaxOrOutliers, threshold);
	...
	private static void readLineToCacheOrPad(Object pixels, int width, int height, int roiY, int xminInside, int widthInside, float[] cache, int cacheWidth, int cacheHeight, int padLeft, int padRight, int colorChannel, int kHeight, int y) {
		...
		if(y < height) {
			readLineToCache(pixels, y * width, xminInside, widthInside, cache, lineInCache * cacheWidth, padLeft, padRight, colorChannel);
			if(y == 0)
	...
	private void resetRoiBoundary(ImageProcessor ip, Rectangle roi, Rectangle roi1) {
		...
		for(int y = roi.y + roi.height,
				p = roi1.x + y * width; y < roi1.y + roi1.height; y++, p += width)
			System.arraycopy(snapshot, p, pixels, p, roi1.width);
	}
	...
	void showMasks() {
		...
		for(double r = 0.5; r < 50; r += 0.5) {
			...
			for(int i = 0, y = y0; i < kHeight; i++, y++)
				for(int x = w / 2 + lineRadii[2 * i],
						p = x + y * w; x <= w / 2 + lineRadii[2 * i + 1]; x++, p++)
					pixels[p] = 1f;
