...
public class XYCoordinates implements PlugIn {
	...
	public void run(String arg) {
		...
		gd.setInsets(0, 20, 0);
		gd.addMessage(msg, null, ij.swt.Color.darkGray);
		...
		boolean isDouble = ip instanceof ij.process.DoubleProcessor;
		for(int z = 0; z < slices; z++) {
			if(slices > 1) {
				ip = stack.getProcessor(z + 1);
				isDouble = ip instanceof ij.process.DoubleProcessor;
			}
			double[] dpixels = isDouble ? (double[])ip.getPixels() : null;
			int w = ip.getWidth();
			String zstr = slices > 1 ? z + "\t" : "";
			for(int i = 0; i < height; i++) {
				int y = invertY ? i : height - 1 - i;
				for(int x = 0; x < width; x++) {
					v = isDouble ? dpixels[y * w + x] : ip.getPixelValue(x, y);
					if(v != background && !(nanBackground && Double.isNaN(v))) {
						if(type == ImagePlus.GRAY32 || isDouble)
			...
			for(int x = 0; x < r.width; x++) {
				if(mask == null || mask.getPixel(x, y) != 0) {
					...
					if(rgb) {
						...
						rt.addValue("Value", dpixels[(r.y + y) * w + (r.x + x)]); // full 64-bit
					else
