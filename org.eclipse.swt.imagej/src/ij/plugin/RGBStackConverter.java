...
public class RGBStackConverter implements PlugIn, DialogListener {
	...
	public void run(String arg) {
		...
		if(cimg == null && !(type == ImagePlus.GRAY8 || type == ImagePlus.GRAY16)) {
			IJ.error("RGB Stack Converter", "8-bit or 16-bit grayscale stack required.\n" + "Use Image > Type to convert from 32-bit / 64-bit first.");
			return;
		}
		if(!imp.lock())
			return;
	...
	public void convertHyperstack(ImagePlus imp, ImagePlus imp2) {
		...
		if(overlay != null) {
			int firstC = c1, lastC = c1, firstZ = z1, lastZ = z1, firstT = t1,
					lastT = t1;
	...
	void compositeImageToRGB(CompositeImage imp, String title) {
		...
		if(imp.getMode() == IJ.COMPOSITE) {
			ImagePlus imp2 = imp.createImagePlus();
			imp.updateImage();
			imp2.setProcessor(title, new ColorProcessor(imp.getImageSwt()));
			// Added by Marcel Boeglin 2013.09.26
			Overlay overlay = imp.getOverlay();
		...
		for(int i = 1; i <= n; i++) {
			...
			stack.addSlice(null, new ColorProcessor(imp.getImageSwt()));
		}
	...
	public boolean dialogItemChanged(GenericDialog gd, TypedEvent e) {
		...
		if(!IJ.isMacro())
			staticKeep = keep;
		((org.eclipse.swt.widgets.Label)gd.getMessage()).setText(getNewDimensions());
		return true;
	}
