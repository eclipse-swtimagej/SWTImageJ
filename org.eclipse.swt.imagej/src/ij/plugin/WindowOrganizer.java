...
public class WindowOrganizer implements PlugIn {
	...
	void tileWindows(int[] wList) {
		Rectangle screen = GUI.getMaxWindowBounds(IJ.getInstance().getShell());
		...
		for (int i = 0; i < nPics; i++) {
			...
			if (win != null) {
				...
				while (win.getShellSize().x * 0.85 >= tileWidth && canvas.getMagnification() > 0.03125) {
					canvas.zoomOut(0, 0);
					win.setShellSize(canvas.getCanvasSize());
				}
				win.toFront();
	...
	void cascadeWindows(int[] wList) {
		...
		for (int i = 0; i < wList.length; i++) {
			...
			if (y + d.y * 0.67 > screen.height) {
				xstart += xstep;
				if (xstart + d.x * 0.67 > screen.width)
