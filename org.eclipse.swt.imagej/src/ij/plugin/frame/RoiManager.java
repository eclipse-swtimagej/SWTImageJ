...
import ij.plugin.frame.swt.YesNoCancelDialog;
...
public class RoiManager extends PlugInFrame implements MouseListener, MouseWheelListener, Iterable<Roi> {
	...
	private org.eclipse.swt.widgets.Button moreButton, colorButton;
	private org.eclipse.swt.widgets.Button showAllCheckbox;
	private org.eclipse.swt.widgets.Button labelsCheckbox;
	...
	/* Embedded mode in a given composite! String is ignored! */
	public RoiManager(String createComposite) {
	...
	void showWindow() {
		...
		if(embedded == false) {
			...
			if(loc != null)
				shell.setLocation(new org.eclipse.swt.graphics.Point(loc.x, loc.y));
			else {
	...
	public void show() {
		...
		shell.layout();
		final org.eclipse.swt.graphics.Point newSize = shell.computeSize(SWT.DEFAULT, SWT.DEFAULT, true);
		shell.setSize(newSize);
		shell.open();
	}

	/* Helper method for the shell! */
	public void setVisible(boolean arg) {

		shell.setVisible(arg);
	}
	/*
	 * void addButton(String label) { Button b = new Button(label);
	 * b.addActionListener(this); b.addKeyListener(IJ.getInstance());
	 * b.addMouseListener(this); if (label.equals(moreButtonLabel)) moreButton = b;
	 * panel.add(b); }
	 */

	void addPopupMenu() {

		popup = new Menu(moreButton);
		moreButton.setMenu(popup);
		// pm = new PopupMenu();
	...
	void addPopupItem(String s) {
		...
		org.eclipse.swt.widgets.MenuItem menuItem_file = new org.eclipse.swt.widgets.MenuItem(popup, SWT.CASCADE);
		// menuItem_file.setMenu(popup);
		menuItem_file.setText(s);
		menuItem_file.addSelectionListener(new SelectionListener() {
	...
	public void actionPerformed(String e) {
		...
		if(command.equals("Add [t]"))
			...
			org.eclipse.swt.graphics.Point bloc = moreButton.getLocation();
	...
	public void itemStateChanged(String item) {
		...
		String source = item;
		boolean showAllMode = showAllCheckbox.getSelection();
		if(source.equals(showAllCheckbox.getText())) {
			if(firstTime && okToSet())
				labelsCheckbox.setSelection(true);
			showAll(showAllCheckbox.getSelection() ? SHOW_ALL : SHOW_NONE);
		...
		if(source.equals(labelsCheckbox.getText())) {
			if(firstTime && okToSet())
				showAllCheckbox.setSelection(true);
			boolean editState = labelsCheckbox.getSelection();
			boolean showAllState = showAllCheckbox.getSelection();
	...
	boolean addRoi(Roi roi, boolean promptForName, Color color, int lineWidth) {
		...
		ImagePlus imp = roi == null ? getImagePlus() : WindowManager.getCurrentImage();
		if(roi == null) {
			if(imp == null)
				return false;
			roi = imp.getRoi();
	 ...
	 *            the Roi to be added
	 * @param n
	 *            if zero or greater, will be used to form the first part of the
	 *            label
	 */
	 ...
	 * 
	 * @param imp
	 *            the image associated with the ROI, or null
	 * @param roi
	 *            the Roi to be added
	 * @param n
	 *            if zero or greater, will be used to form the first part of the
	...
	boolean delete(boolean replacing) {
		...
		if(index.length == 0 || (replacing && count > 1)) {
			...
			canceled = false;
			if(!IJ.isMacro() && !macro) {
				YesNoCancelDialog d = new YesNoCancelDialog(shell, "ROI Manager", msg);
				if(d.cancelPressed()) {
					canceled = true;
					return false;
				}
	...
	boolean restore(ImagePlus imp, int index, boolean setSlice) {
		...
		dis.syncExec(new Runnable() {
			...
			public void run() {
				...
				selection.set(showAllCheckbox.getSelection());
			}
		});
	 ...
	 * per slice.
	 * 
	 * @see <a href="http://imagej.net/ij/macros/js/MultiMeasureDemo.js">JavaScript
	 *      example</a>
	 */
	...
	void multiPlot() {
		...
		for(int i = 0; i < indexes.length; i++) {
			if(!restore(getImagePlus(), indexes[i], true))
				break;
			Roi roi = imp.getRoi();
			if(roi == null)
				break;
	...
	private void multiCrop() {
		...
		gd.addMessage("drag and drop target", Display.getDefault().getSystemFont(), Display.getDefault().getSystemColor(SWT.COLOR_DARK_GRAY));
	...
	public void multiCrop(String directory, String options) {
		...
		if(options.contains("show")) {
			...
			ImagePlus imgStack = new ImagePlus("CROPPED_" + text, stack);
	...
	public boolean getDrawLabels() {
		...
		return labelsCheckbox.getSelection();
	}
	...
	private static boolean channel = false, slice = true, frame = false; // remembers checkbox states for
																			// removePositions
	...
	private void removePositions(int position) {
		...
		if(position == SHOW_DIALOG) {
			...
			gd.setInsets(5, 15, 0);
			gd.addMessage("Remove positions for:      ", Display.getDefault().getSystemFont());
			gd.setInsets(6, 25, 0);
	...
	/*
	 * Panel makeButtonPanel(GenericDialogSwt gd) { Panel panel = new Panel();
	 * //buttons.setLayout(new FlowLayout(FlowLayout.CENTER, 5, 0)); colorButton =
	 * new Button("\"Show All\" Color..."); colorButton.addActionListener(this);
	 * panel.add(colorButton); return panel; }
	 */
	...
	/* Renamed for SWT. Shell has already getImage method! */
	...
	boolean error(String msg) {
		...
		new ij.plugin.frame.swt.MessageDialog(shell, "ROI Manager", msg);
		Macro.abort();
		return false;
	}
	...
	public void processWindowEvent(WindowEvent e) {
		...
		/* To do for SWT. Port this function! */
	 ...
	 */
	/*
	 * public List getList() { List awtList = new List(); for (int i = 0; i <
	 * getCount(); i++) awtList.add((String) list.getItem(i)); int index =
	 * getSelectedIndex(); if (index >= 0) awtList.select(index); return awtList; }
	 */
	 ...
	 */
	/*
	 * Changed listModel for SWT List!
	 */
	...
	public boolean runCommand(String cmd, String name) {
		...
		if(cmd.equals("open")) {
			...
			macro = false;
			if(labelsCheckbox.getSelection()) {
				ImagePlus imp = WindowManager.getCurrentImage();
				if(imp != null)
					imp.draw();
			}
	...
	public void select(ImagePlus imp, int index) {
		...
		if(index < 0)
			return;
		/*
		 * } int n = getCount(); if(index >= n) return; Changed for SWT! int delay = 1;
		 * long start = System.currentTimeMillis(); AtomicReference<Boolean> selected =
		 * new AtomicReference<Boolean>(); Display dis = Display.getDefault();
		 * dis.syncExec(new Runnable() { public void run() { while(true) {
		 * selected.set(list.isSelected(index)); if(selected.get()) { break; }
		 * list.deselectAll(); list.setSelection(index); } } });
		 */
	 ...
	 * function!
	 */
	/** Overrides PlugInFrame.closeSwt(). */
	public void close() {
	...
	public boolean isSelected(int index) {
		...
		return index >= 0 && index < list.getItemCount() && list.isSelected(index);
	}
	...
	private Overlay newOverlay() {
		...
		dis.syncExec(new Runnable() {
			...
			public void run() {
				...
				selection.set(labelsCheckbox.getSelection());
			}
		});
		overlay.drawLabels(selection.get());
	...
	public String toString() {
		...
		return "RoiManager[size=" + getCount() + ", visible=" + shell.isVisible() + "]";
	}
	 ...
	 * !e.isConsumed()) { e.consume(); IJ.run("ROI Manager Action"); } }
	 */
