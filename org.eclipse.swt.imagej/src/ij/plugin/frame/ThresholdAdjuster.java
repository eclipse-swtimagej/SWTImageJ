 ...
 */
public class ThresholdAdjuster extends PlugInDialog implements PlugIn, Measurements, Runnable, SelectionListener, org.eclipse.swt.events.FocusListener, org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.MouseWheelListener, ImageListener {
	...
	org.eclipse.swt.widgets.Button darkBackgroundCheckbox, stackCheckbox,
			noResetCheckbox, rawValues, sixteenBitCheckbox;
	...
	void updateLabels(ImagePlus imp, ImageProcessor ip) {
		...
		Display.getDefault().syncExec(() -> {
			...
			if(min == ImageProcessor.NO_THRESHOLD) {
				...
				if((((int)min == min && (int)max == max && Math.abs(min) < 1e6 && Math.abs(max) < 1e6)) || (ip instanceof ShortProcessor && (cal.isSigned16Bit() || !calibrated))) {
	...
	static final int RESET = 0, AUTO = 1, HIST = 2, APPLY = 3, STATE_CHANGE = 4,
			MIN_THRESHOLD = 5, MAX_THRESHOLD = 6, SET = 7;
...
class ThresholdPlot extends org.eclipse.swt.widgets.Canvas implements Measurements, org.eclipse.swt.events.MouseListener, PaintListener {
