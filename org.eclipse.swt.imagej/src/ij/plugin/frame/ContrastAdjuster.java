...
public class ContrastAdjuster extends PlugInDialog implements Runnable, SelectionListener, ItemListener {
	...
	boolean RGBImage;
	org.eclipse.swt.widgets.Slider minSlider, maxSlider, contrastSlider, brightnessSlider;
	...
	void apply(ImagePlus imp, ImageProcessor ip) {
		...
		if (imp.getStackSize() > 1 && !imp.isComposite()) {
			...
			YesNoCancelDialog d = new YesNoCancelDialog("Entire Stack?",
					"Apply LUT to all " + stack.size() + " stack slices?");
			if (d.cancelPressed()) {
				imp.unlock();
				return;
			}
	...
	private void propagate(ImagePlus img) {
		...
		if (img.isComposite()) {
			...
			for (int i = 0; i < nImages; i++) {
				ImagePlus img2 = WindowManager.getImage(list[i]);
				if (img2 != null && img2.getBitDepth() == depth && img2.getID() != id && img2.getNChannels() == 1
						&& img2.getWindow() != null) {
