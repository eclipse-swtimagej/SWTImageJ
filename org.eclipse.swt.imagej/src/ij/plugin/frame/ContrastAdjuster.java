 ...
 */
public class ContrastAdjuster extends PlugInDialog implements Runnable, SelectionListener, ItemListener {
	...
	int minSliderValue = -1, maxSliderValue = -1, brightnessValue = -1,
			contrastValue = -1;
	int sliderRange = 256;
	boolean doAutoAdjust, doReset, doSet, doApplyLut;
	org.eclipse.swt.widgets.Composite panel, tPanel;
	org.eclipse.swt.widgets.Button autoB, resetB, setB, applyB;
	int previousImageID;
	int previousType;
	int previousSlice = 1;
	ImageJ ij;
	...
	boolean RGBImage;
	org.eclipse.swt.widgets.Slider minSlider, maxSlider, contrastSlider,
			brightnessSlider;
	org.eclipse.swt.widgets.Label minLabel, maxLabel, windowLabel, levelLabel;
	boolean done;
	int autoThreshold;
	...
	org.eclipse.swt.graphics.Font monoFont = new org.eclipse.swt.graphics.Font(Display.getDefault(), new FontData("Monospaced", 8, SWT.NORMAL));
	...
	public void run(String arg) {
		...
		Display.getDefault().syncExec(() -> {
			...
			instance = ContrastAdjuster.this;
	...
	void addLabel(String text, org.eclipse.swt.widgets.Label label2) {
	...
	public synchronized void adjustmentValueChanged(SelectionEvent e) {
		...
		if(source == minSlider)
			minSliderValue = minSlider.getSelection();
		else if(source == maxSlider)
			maxSliderValue = maxSlider.getSelection();
		else if(source == contrastSlider)
			contrastValue = contrastSlider.getSelection();
		else
			brightnessValue = brightnessSlider.getSelection();
		notify();
	}
	...
	void updateScrollBars(org.eclipse.swt.widgets.Slider sb, boolean newRange) {
		...
		Display.getDefault().syncExec(() -> {
			if(sb == null || sb != contrastSlider) {
				...
				if(contrastSlider != null) {
					if(newRange)
						contrastSlider.setValues(contrast, 0, sliderRange, 1, 1, 1);
					else {
						Display.getDefault().syncExec(() -> {
							contrastSlider.setSelection(contrast);
						});
					}
				}
			}
			if(sb == null || sb != brightnessSlider) {
				...
				if(newRange)
					brightnessSlider.setValues(brightness, 0, sliderRange, 1, 1, 1);
				else
					brightnessSlider.setSelection(brightness);
			}
			if(minSlider != null && (sb == null || sb != minSlider)) {
				if(newRange)
					minSlider.setValues(scaleDown(min), 0, sliderRange, 1, 1, 1);
				else
					minSlider.setSelection(scaleDown(min));
			}
			if(maxSlider != null && (sb == null || sb != maxSlider)) {
				if(newRange)
					maxSlider.setValues(scaleDown(max), 0, sliderRange, 1, 1, 1);
				else
					maxSlider.setSelection(scaleDown(max));
			}
		});
	}
	...
	void apply(ImagePlus imp, ImageProcessor ip) {
		...
		if(imp.getStackSize() > 1 && !imp.isComposite()) {
			...
			YesNoCancelDialog d = new YesNoCancelDialog("Entire Stack?", "Apply LUT to all " + stack.size() + " stack slices?");
			if(d.cancelPressed()) {
				imp.unlock();
				return;
			}
	...
	void applyRGB(ImagePlus imp, ImageProcessor ip) {
		 ...
		 * (Recorder.record) { if (Recorder.scriptMode())
	...
	static final int RESET = 0, AUTO = 1, SET = 2, APPLY = 3, THRESHOLD = 4,
			MIN = 5, MAX = 6, BRIGHTNESS = 7, CONTRAST = 8, UPDATE = 9;
	...
	public void shellActivated(ShellEvent e) {
		...
		/*
		 * Window owin = e.getOppositeWindow(); if (owin == null || !(owin instanceof
		 * ImageWindow)) return;
		 */
		/*
...
class ContrastPlot extends org.eclipse.swt.widgets.Canvas implements org.eclipse.swt.events.MouseListener, PaintListener {
	...
	org.eclipse.swt.graphics.Color color = ij.swt.Color.gray;
	double scale = Prefs.getGuiScale();
	int width = WIDTH;
	int height = HEIGHT;
	private GC gcos;

	public ContrastPlot(Shell shell) {
	...
	public void paint(GC gc) {
		...
		gc.drawLine(x2, height - 5, x2, height);
...
/*
 * class TrimmedLabel extends org.eclipse.swt.widgets.Label { int trim =
 * IJ.isMacOSX()?0:6; public TrimmedLabel(String title) { super(title); } public
 * Dimension getMinimumSize() { return new
 * Dimension(super.getMinimumSize().width, super.getMinimumSize().height-trim);
