...
public class Editor extends PlugInFrame implements WindowSwt, SelectionListener, org.eclipse.swt.events.ModifyListener, org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.MouseListener, ClipboardOwner, MacroConstants, Runnable, Debugger {
	...
	public Editor(int rows, int columns, int fontSize, int options, boolean contextMenu, boolean embedded) {
		...
		Display.getDefault().syncExec(() -> {
			...
			/*
			 * Create the first menus and menu items. The Macros and Debug menus will
			 * be created when opening a specific file (see method 'create')!
			 */
			addMenuBar(options, mb);
			...
			if(contextMenu) {
				mb = new Menu(ta.getShell(), SWT.POP_UP);
				addMenuBar(options, mb);
				ta.setMenu(mb);
			}
	...
	void addMenuBar(int options, Menu mb) {
