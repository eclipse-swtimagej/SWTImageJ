...
public class Fitter extends PlugInFrame
	...
	public Fitter(double[] dx, double[] dy) {
		...
		Display.getDefault().syncExec(() -> {
			...
			// font = new org.eclipse.swt.graphics.Font(Display.getDefault(),"SansSerif",14,
			// SWT.NORMAL);
	...
	private void paste() {
		...
		/*
		 * Clipboard clipboard = getToolkit().getSystemClipboard(); Transferable
		 * clipData = clipboard.getContents(s); try { s = (String)
		 * (clipData.getTransferData(DataFlavor.stringFlavor)); } catch (Exception e) {
		 * s = e.toString(); }
		 */
		/*
		 * int start = textArea.getSelectionStart(); int end =
		 * textArea.getSelectionEnd(); textArea.replaceRange(s, start, end);
		 */
		int start = textArea.getSelectionRange().x;
		int end = textArea.getSelection().y;
