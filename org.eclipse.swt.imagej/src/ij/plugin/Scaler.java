...
public class Scaler implements PlugIn, ModifyListener, org.eclipse.swt.events.FocusListener {
	...
	Text xField;
	private org.eclipse.swt.widgets.Text yField, zField, widthField, heightField, depthField;
	private Rectangle r;
	 ...
	 * contain 'none', 'bilinear'. 'bicubic', 'slice' and 'constrain'.
	 */
	public static ImagePlus resize(ImagePlus imp, int dstWidth, int dstHeight, int dstDepth, String options) {
		...
		// return new ImagePlus("Untitled", ip.resize(dstWidth, dstHeight,
		// useAveraging));
		Roi roi = imp.getRoi();
		ImageProcessor ip = imp.getProcessor();
		if (roi != null && !roi.isArea())
	...
	public static void record(ImagePlus imp, int w2, int h2, int d2, int method) {
		...
		if (method == ImageProcessor.NONE)
			...
			options = "bilinear";
		Recorder.recordCall("imp = imp.resize(" + w2 + ", " + h2 + (d2 > 0 && d2 != imp.getStackSize() ? ", " + d2 : "")
				+ ", \"" + options + "\");");
	}
	...
	boolean showDialog(ImageProcessor ip) {
		...
		if (fields != null) {
			...
			if (isStack) {
				zField = (org.eclipse.swt.widgets.Text) fields.elementAt(2);
				widthField = (org.eclipse.swt.widgets.Text) fields.elementAt(3);
				heightField = (org.eclipse.swt.widgets.Text) fields.elementAt(4);
				depthField = (org.eclipse.swt.widgets.Text) fields.elementAt(5);
			} else {
				widthField = (org.eclipse.swt.widgets.Text) fields.elementAt(2);
				heightField = (org.eclipse.swt.widgets.Text) fields.elementAt(3);
			}
		}
