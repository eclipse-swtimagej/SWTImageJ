...
public class Slicer implements PlugIn, ModifyListener, SelectionListener {
	...
	private org.eclipse.swt.widgets.Label message;
	private ImagePlus imp;
	private double gx1, gy1, gx2, gy2, gLength;
	private org.eclipse.swt.graphics.Color lineColor = new org.eclipse.swt.graphics.Color(255, 255, 0, 102);
	...
	public void run(String arg) {
		...
		if (roi != null && roiType != Roi.RECTANGLE && roiType != Roi.LINE && roiType != Roi.POLYLINE
	...
	ImagePlus resliceHyperstack(ImagePlus imp) {
		...
		for (int t = 1; t <= frames; t++) {
			for (int c = 1; c <= channels; c++) {
				...
				if (imp2 == null) {
					imp2 = tmp2.createHyperStack("Reslice of " + imp.getTitle(), channels, slices2, frames,
							tmp2.getBitDepth());
	...
	boolean showDialog(ImagePlus imp) {
		...
		gd.setInsets(15, 0, 0);
		gd.addMessage("Voxel size: " + d2s(cal.pixelWidth) + "x" + d2s(cal.pixelHeight) + "x" + d2s(cal.pixelDepth)
				+ " " + cal.getUnit());
		gd.setInsets(5, 0, 0);
	...
	void drawLine(double x1, double y1, double x2, double y2, ImagePlus imp) {
		...
		dis.syncExec(new Runnable() {
			public void run() {
				...
				gc.drawLine(ic.screenX((int) (x1 + 0.5)), ic.screenY((int) (y1 + 0.5)), ic.screenX((int) (x2 + 0.5)),
						ic.screenY((int) (y2 + 0.5)));
			}
		});
	}
	...
	void updateSize() {
		// double inSpacing =
		// Tools.parseDouble(((TextField)fields.elementAt(0)).getText(),0.0);
	...
	String getSize(double inSpacing, double outSpacing, int count) {
		...
		if (message != null)
			message.setForeground(mem != -1 && size > mem ? ij.swt.Color.red : ij.swt.Color.black);
		if (size > 0)
	...
	public void widgetSelected(SelectionEvent e) {
		if (IJ.isMacOSX())
			IJ.wait(100);
		org.eclipse.swt.widgets.Button cb = (org.eclipse.swt.widgets.Button) checkboxes.elementAt(2);
		nointerpolate = cb.getSelection();
		updateSize();
