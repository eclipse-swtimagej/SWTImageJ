...
public class Slicer implements PlugIn, ModifyListener, SelectionListener {
	...
	public void run(String arg) {
		...
		int sliceType = imp.getType();
		notFloat = !rgb && sliceType != ImagePlus.GRAY32 && sliceType != ImagePlus.GRAY64;
		if(imp.isHyperStack())
	...
	boolean showDialog(ImagePlus imp) {
		...
		gd.setInsets(15, 0, 0);
		gd.addMessage("Voxel size: " + d2s(cal.pixelWidth) + "x" + d2s(cal.pixelHeight) + "x" + d2s(cal.pixelDepth) + " " + cal.getUnit());
		gd.setInsets(5, 0, 0);
	...
	void drawLine(double x1, double y1, double x2, double y2, ImagePlus imp) {
		...
		dis.syncExec(new Runnable() {
			...
			public void run() {
				...
				gc.drawLine(ic.screenX((int)(x1 + 0.5)), ic.screenY((int)(y1 + 0.5)), ic.screenX((int)(x2 + 0.5)), ic.screenY((int)(y2 + 0.5)));
			}
		});
	}
	/*
