...
public class RoiEnlarger implements PlugIn, DialogListener {
	...
	private org.eclipse.swt.widgets.Label unitsLabel;
	...
	public double showDialog(ImagePlus imp, double pixels) {
		...
		unitsLabel = (org.eclipse.swt.widgets.Label)gd.getMessage();
		if (scaled) {
			gd.setInsets(0, 20, 0);     //top left bottom
			gd.addCheckbox("Pixel units", usePixels);
		}
		gd.setInsets(10, 0, 0);
		gd.addMessage("Enter negative number to shrink", null, ij.swt.Color.darkGray);
		if (xyScaleDifferent) {
			gd.setInsets(5, 0, 0);
			gd.addMessage(" \n ", null, ij.swt.Color.red);
		}
	...
	public boolean dialogItemChanged(GenericDialog gd, TypedEvent e) {
		...
		org.eclipse.swt.widgets.Button usePixelsCbx = checkboxes == null ? null : (org.eclipse.swt.widgets.Button)checkboxes.get(0);
		...
		if (e!=null && e.getSource() == usePixelsCbx) {
			...
			org.eclipse.swt.widgets.Text numberField = (org.eclipse.swt.widgets.Text)gd.getNumericFields().get(0);
	...
	private void updateWarning(GenericDialog gd) {
		org.eclipse.swt.widgets.Button usePixelsCbx = (org.eclipse.swt.widgets.Button)gd.getCheckboxes().get(0);
		org.eclipse.swt.widgets.Label warningLabel = (org.eclipse.swt.widgets.Label)gd.getMessage();
		boolean showWarning = !usePixelsCbx.getSelection(); //warn if not pixels units
