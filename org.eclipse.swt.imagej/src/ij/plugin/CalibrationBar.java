...
public class CalibrationBar implements PlugIn {
	...
	static final String[] colors = { "White", "Light Gray", "Dark Gray", "Black", "Red", "Green", "Blue", "Yellow",
			"None" };
	...
	private void updateColorBar() {
		...
		if (roi != null && location.equals(locations[AT_SELECTION])) {
			...
			drawBarAsOverlay(imp, insetPad,
					imp.getHeight() - (int) (WIN_HEIGHT * zoom + 2 * (int) (YMARGIN * zoom)) - (int) (insetPad * zoom));
	...
	private boolean showDialog() {
		...
		org.eclipse.swt.widgets.Button overlayBox = (org.eclipse.swt.widgets.Button) (gd.getCheckboxes().elementAt(1));
	...
	private void drawBarAsOverlay(ImagePlus imp, int x, int y) {
		...
		win_width = (int) (XMARGIN * zoom) + 5 + (int) (BAR_THICKNESS * zoom) + maxTextWidth
				+ (int) ((XMARGIN / 2) * zoom);
		if (x == -1 && y == -1)
	...
	class LiveDialog extends GenericDialog {
		...
		public void textValueChanged(ModifyEvent e) {
			...
			if (fieldNames == null) {
				...
				for (int i = 0; i < 4; i++) {
					fieldNames[i] = ((org.eclipse.swt.widgets.Text) numberField.elementAt(i)).getMessage();
				System.out.println(fieldNames[i]);
				}
			}

			org.eclipse.swt.widgets.Text tf = (org.eclipse.swt.widgets.Text) e.getSource();
			String name =tf.getMessage();
		...
		public void itemStateChanged(SelectionEvent e) {
			Combo combo = (org.eclipse.swt.widgets.Combo) (choice.elementAt(0));
			location = combo.getItem(combo.getSelectionIndex());
			Combo combo2 = (Combo) (choice.elementAt(1));
			fillColor = combo2.getItem(combo2.getSelectionIndex());
			Combo combo3 = (Combo) (choice.elementAt(2));
			textColor = combo3.getItem(combo3.getSelectionIndex());
			boldText = ((org.eclipse.swt.widgets.Button) (checkbox.elementAt(0))).getSelection();
			flatten = !((org.eclipse.swt.widgets.Button) (checkbox.elementAt(1))).getSelection();
			showUnit = ((org.eclipse.swt.widgets.Button) (checkbox.elementAt(2))).getSelection();
			org.eclipse.swt.widgets.Button overlayBox = (org.eclipse.swt.widgets.Button) (checkbox.elementAt(1));
			if (location.equals(locations[SEPARATE_IMAGE]))
				overlayBox.setEnabled(false);
			else
