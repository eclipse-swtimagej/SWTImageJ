...
public class PixelInspectionTool extends PlugInTool {
	...
	void drawOutline(ImagePlus imp, MouseEvent e) {
		...
		int x = ic.offScreenX(e.x);
		int y = ic.offScreenY(e.y);
...
class PixelInspector extends PlugInFrame implements ImageListener, org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.MouseListener, Runnable {
	...
	org.eclipse.swt.widgets.Label[] labels; // the display fields
	private org.eclipse.swt.widgets.Label prefsLabel;
	int size = 2 * radius + 2; // number of columns and rows
	// Label prefsLabel = new Label("Prefs\u2026");
	// org.eclipse.swt.widgets.Label prefsLabel = new
	...
	private void init() {
		...
		for(int i = 0; i < labels.length; i++) { // make the labels (display fields)
			if(i == 0) {
				...
				labels[i] = new org.eclipse.swt.widgets.Label(shell, SWT.NONE);
			}
		}
	...
	void initializeLabels() {
		...
		org.eclipse.swt.graphics.Color bgColor = Display.getDefault().getSystemColor(SWT.COLOR_DARK_GRAY);
	...
	/**
	 * Converts a number to a string in decimal or exp format. The number of digits
	 * is chosen to make the value fit into a cell the size of "000000.00"
	 */
	...
	void showDialog() {
		...
		org.eclipse.swt.graphics.Point location = gd.getShell().getLocation();
