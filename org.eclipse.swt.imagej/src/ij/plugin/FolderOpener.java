 ...
 * images as a stack.
 */
public class FolderOpener implements PlugIn, ModifyListener {
	...
	private org.eclipse.swt.widgets.Text dirField, filterField, startField, countField, stepField;

	/**
	 * Opens the images in the specified directory as a stack. Displays directory
	 * chooser and options dialogs if the argument is null.
	 */
	...
	/**
	 * Opens the images in the specified directory as a stack. Opens the images as a
	 * virtual stack if the 'options' string contains 'virtual' or 'use'. Add '
	 * filter=abc' to the options string to only open images with, for example,
	 * 'abc' in their name. The image type, start, step, count and scale can also be
	 * set, for example "bitdepth=32 start=10 step=2 count=10 scale=50". Add '
	 * noMetaSort' to disable sorting of DICOM stacks by series number (0020,0011).
	 * Displays the Import/Sequence dialog if the the 'path' argument is null.
	 */
	...
	/**
	 * Opens the images in the specified directory as a widthxheight stack. Opens
	 * the images as a virtual stack if the 'options' string contains 'virtual'. Add
	 * ' filter=abc' to the options string to only open images with, for example,
	 * 'abc' in their name. The image type, start, step, count and scale can also be
	 * set, for example "bitdepth=32 start=10 step=2 count=10 scale=50".
	 */
			...
			if ((arg == null || runningOpen) && !saveImage) {
				...
				if (openAsSeparateImages) {
					if (imp2.getStackSize() > MAX_SEPARATE && !IJ.isMacro()) {
						boolean ok = IJ.showMessageWithCancel("Import>Image Sequence", "Are you sure you want to open "
								+ imp2.getStackSize()
	...
	boolean showDialog() {
		...
		gd.setInsets(0, 55, 0);
		gd.addMessage("enclose regex in parens", IJ.font10, ij.swt.Color.darkGray);
