 ...
 */
public class Distribution implements PlugIn, ModifyListener {
	...
	org.eclipse.swt.widgets.Text nBinsField, rangeField;
	...
	public void run(ResultsTable rt) {
		...
		gd.setInsets(0, 40, 0);
		gd.addMessage(count + " data points", null, ij.swt.Color.darkGray);
		...
		if(v != null) {
			nBinsField = (org.eclipse.swt.widgets.Text)v.elementAt(0);
			nBinsField.addModifyListener(this);
		}
		v = gd.getStringFields();
		if(v != null) {
			rangeField = (org.eclipse.swt.widgets.Text)v.elementAt(0);
			rangeField.addModifyListener(this);
		}
	...
	void stats(int nc, float[] data, float[] pars) {
		...
		int i;
		float s = 0, min = Float.MAX_VALUE, max = -Float.MAX_VALUE, totl = 0,
				ave = 0, adev = 0, sdev = 0, var = 0, skew = 0, kurt = 0, p;
		for(i = 0; i < nc; i++) {
			totl += data[i];
			// tot& = tot& + 1
			if(data[i] < min)
