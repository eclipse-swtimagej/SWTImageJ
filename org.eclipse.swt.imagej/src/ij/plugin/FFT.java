...
public class FFT implements PlugIn, Measurements {
	...
	public void run(String arg) {
		...
		if(imp == null)
			imp = IJ.getImage();
		// FFT/FHT operates on 32-bit float internally and is not supported for
		// 64-bit (double) images. Refuse forward transforms with a clear message.
		// (Inverse transforms act on the float FHT/complex stack and are unaffected.)
		boolean inverseArg = arg.equals("inverse") || arg.equals("redisplay") || arg.equals("swap");
		if(!inverseArg && imp != null && imp.getBitDepth() == 64) {
			IJ.error("FFT", "FFT is not supported for 64-bit (double) images.\n" + "Please convert to 32-bit first: Image\u25b8Type\u25b832-bit");
			return;
		}
	 ...
	 * 
	 * @param imp
	 *            A spatial domain image, which is not modified
	 ...
	 * 
	 * @param imp
	 *            A frequency domain image, which is modified.
	 * @param filter
	 *            The filter, 32-bits (0-1) or 8-bits (0-255)
	 * @see #forward
	 * @see #inverse
	 * @see #filter
	 */
	...
	static void smooth(ImageProcessor ip) {
		...
		for(int y = 0; y < n; y++) {
			...
			for(int x = 0; x < n; x++) {
				int sum = (pixels2[offset1 + iMinus[x]] & 255) + (pixels2[offset1 + x] & 255) + (pixels2[offset1 + iPlus[x]] & 255) + (pixels2[offset2 + iMinus[x]] & 255) + (pixels2[offset2 + x] & 255) + (pixels2[offset2 + iPlus[x]] & 255) + (pixels2[offset3 + iMinus[x]] & 255) + (pixels2[offset3 + x] & 255) + (pixels2[offset3 + iPlus[x]] & 255);
