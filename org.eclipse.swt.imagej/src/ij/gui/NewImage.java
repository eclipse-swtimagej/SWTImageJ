...
public class NewImage {
	...
	public static final int GRAY8 = 0, GRAY16 = 1, GRAY32 = 2, RGB = 3,
			GRAY64 = 4;
	public static final int FILL_BLACK = 1, FILL_RAMP = 2, FILL_NOISE = 3,
			FILL_RANDOM = 3, FILL_WHITE = 4, CHECK_AVAILABLE_MEMORY = 8,
			SIGNED_INT = 16;
	...
	public static ImagePlus createDoubleImage(String title, int width, int height, int slices, int options) {
		...
		ImagePlus imp = new ImagePlus(title, ip);
		if(slices > 1) {
			boolean ok = createStack(imp, ip, slices, GRAY64, options);
			if(!ok)
				imp = null;
		}
		if(fill != FILL_NOISE)
	...
	private static void fillNoiseDouble(ImageProcessor ip) {
	...
	public static ImagePlus createImage(String title, int width, int height, int nSlices, int bitDepth, int options) {
		...
		switch(bitDepth) {
			...
			case 32:
				...
				break;
			case 64:
				imp = createDoubleImage(title, width, height, nSlices, options);
				break;
			case 24:
