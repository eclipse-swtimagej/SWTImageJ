...
public class OvalRoi extends Roi {
	...
	protected void moveHandle(int sx, int sy) {
		...
		int x1 = x, y1 = y, x2 = x + width, y2 = y + height, xc = x + width / 2,
				yc = y + height / 2;
	...
	public void draw(Graphics g) {
		...
		/* Also calculate aspectRatio. 1.0 for the normal image view! */
		ic.calculateAspectRatio();
		int sw = (int)(width * mag * ic.aspectRatioX);
		int sh = (int)(height * mag * ic.aspectRatioY);
		int sx1 = (int)(screenX(x) * ic.aspectRatioX);
		int sy1 = (int)(screenY(y) * ic.aspectRatioY);
		if(subPixelResolution() && bounds != null) {
			sw = (int)(bounds.width * mag * ic.aspectRatioX);
			sh = (int)(bounds.height * mag * ic.aspectRatioY);
			sx1 = screenXD((int)(bounds.x * ic.aspectRatioX));
			sy1 = screenYD((int)(bounds.y * ic.aspectRatioY));
		}
		int sw2 = (int)(0.14645 * width * mag * ic.aspectRatioX);
		int sh2 = (int)(0.14645 * height * mag * ic.aspectRatioY);
		int sx2 = sx1 + sw / 2;
		int sy2 = sy1 + sh / 2;
		int sx3 = sx1 + sw;
		int sy3 = sy1 + sh;
	 ...
	 *            determines whether to use image pixel coordinates
