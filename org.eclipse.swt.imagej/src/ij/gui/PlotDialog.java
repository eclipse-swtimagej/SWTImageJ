...
public class PlotDialog implements DialogListener {
	...
	private static final String[] HEADINGS = new String[] { "Plot Range", "Axis Options", "Add Legend",
			"High-Resolution Plot", "Use Template", null, null, null, null, null, // 5-9 spare
			"X Left", "X Right", "Y Bottom", "Y Top", "X Axis", "Y Axis" };
	/** Positions and corresponding codes for legend position */
	private static final String[] LEGEND_POSITIONS = new String[] { "Auto", "Top-Left", "Top-Right", "Bottom-Left",
			"Bottom-Right", "No Legend" };
	private static final int[] LEGEND_POSITION_N = new int[] { Plot.AUTO_POSITION, Plot.TOP_LEFT, Plot.TOP_RIGHT,
			Plot.BOTTOM_LEFT, Plot.BOTTOM_RIGHT, 0 };
	/**
	...
	private static final int[] TEMPLATE_FLAGS = new int[] { Plot.X_RANGE, Plot.Y_RANGE, Plot.COPY_AXIS_STYLE,
			Plot.COPY_LABELS, Plot.COPY_LEGEND, Plot.COPY_CONTENTS_STYLE, Plot.COPY_EXTRA_OBJECTS, Plot.COPY_SIZE };
	...
	private org.eclipse.swt.widgets.Button xLogCheckbox, yLogCheckbox;
	...
	public void showDialog(Shell parent) {
		...
		String dialogTitle = dialogType >= X_LEFT && dialogType <= Y_TOP ? "Set Axis Limit..."
				: (HEADINGS[dialogType] + "...");
	...
	private boolean setupDialog(GenericDialog gd) {
		...
		int xDigits = plot.logXAxis ? -2
				: Plot.getDigits(currentMinMax[0], currentMinMax[1],
						0.005 * Math.abs(currentMinMax[1] - currentMinMax[0]), 6, 0);
		...
		int yDigits = plot.logYAxis ? -2
				: Plot.getDigits(currentMinMax[2], currentMinMax[3],
						0.005 * Math.abs(currentMinMax[3] - currentMinMax[2]), 6, 0);
		...
		if (dialogType == AXIS_OPTIONS || dialogType == X_AXIS || dialogType == Y_AXIS) {
			...
			final String[] labels = new String[] { " Draw Grid", " Major Ticks", " Minor Ticks",
					" Ticks if Logarithmic", " Numbers" };
			final int[] xFlags = new int[] { Plot.X_GRID, Plot.X_TICKS, Plot.X_MINOR_TICKS, Plot.X_LOG_TICKS,
					Plot.X_NUMBERS };
			...
			if ((plotXLabel == null || plotXLabel.equals("Distance (pixels)") || plotXLabel.equals("Distance ( )"))
					&& lastXLabel != null)
			...
			int nChars = 20;
			if ((plotXLabel != null && plotXLabel.startsWith("{"))
					|| (plotYLabel != null && plotYLabel.startsWith("{"))) {
				nChars = Math.max(nChars, plotXLabel.length());
				nChars = Math.max(nChars, plotYLabel.length());
			}
			if (nChars > 80)
				nChars = 80;
			// plotXLabel = plotXLabel.replace("\n", "|"); //multiline label currently no
			// supported by Plot class
			if (dialogType == AXIS_OPTIONS || dialogType == X_AXIS)
		...
		if (dialogType == SET_RANGE || dialogType == X_AXIS || dialogType == Y_AXIS) {
			org.eclipse.swt.graphics.Font smallFont = IJ.font10;
			gd.setInsets(10, 0, 0); // top, left, bottom
			gd.addMessage("*   Leave empty for automatic range", smallFont, ij.swt.Color.darkGray);
			gd.setInsets(0, 0, 0);
			gd.addMessage("** Requires limits > 0 and max/min > 3", smallFont, ij.swt.Color.darkGray);
			if (dialogType == X_AXIS || dialogType == Y_AXIS) {
				gd.setInsets(0, 0, 0);
				gd.addMessage("    Label supports !!sub-!! and ^^superscript^^", smallFont, ij.swt.Color.darkGray);
			}
		}
		...
		if (dialogType == LEGEND) {
			...
			gd.addMessage(msg, smallFont, ij.swt.Color.darkGray);
			gd.setInsets(0, 0, 0);
	...
	private void record() {
		...
		if (dialogType == SET_RANGE || dialogType == X_AXIS)
			Recorder.recordString(plotDot + (Recorder.scriptMode() ? "setAxisXLog(" : "setLogScaleX(")
					+ plot.hasFlag(Plot.X_LOG_NUMBERS) + ");\n");
		if (dialogType == SET_RANGE || dialogType == Y_AXIS)
			Recorder.recordString(plotDot + (Recorder.scriptMode() ? "setAxisYLog(" : "setLogScaleY(")
					+ plot.hasFlag(Plot.Y_LOG_NUMBERS) + ");\n");
		if (dialogType == SET_RANGE || dialogType == X_AXIS || dialogType == Y_AXIS) {
			double[] currentMinMax = plot.getLimits();
			int xDigits = plot.logXAxis ? -2
					: Plot.getDigits(currentMinMax[0], currentMinMax[1],
			...
			int yDigits = plot.logYAxis ? -2
					: Plot.getDigits(currentMinMax[2], currentMinMax[3],
							0.005 * Math.abs(currentMinMax[3] - currentMinMax[2]), 6, 0);
			Recorder.recordString(plotDot + "setLimits(" + IJ.d2s(currentMinMax[0], xDigits) + ","
					+ IJ.d2s(currentMinMax[1], xDigits) + "," + IJ.d2s(currentMinMax[2], yDigits) + ","
		...
		if (dialogType == AXIS_OPTIONS || dialogType == X_AXIS || dialogType == Y_AXIS) {
			...
			if (labelFont != null) {
				if (Recorder.scriptMode())
					Recorder.recordString("plot.setAxisLabelFont(" + (labelFont.isBold() ? "Font.BOLD," : "Font.PLAIN,")
							+ IJ.d2s(labelFont.getSize2D(), 1) + ");\n");
				else
					Recorder.recordString("Plot.setAxisLabelSize(" + IJ.d2s(labelFont.getSize2D(), 1) + ", \""
							+ (labelFont.isBold() ? "bold" : "plain") + "\");\n");
			}
			if (numberFont != null)
				Recorder.recordString(plotDot + (Recorder.scriptMode() ? "setFont(-1, " : "setFontSize(")
						+ IJ.d2s(numberFont.getSize2D(), 1) + ");\n");
			Recorder.recordString(plotDot + "setXYLabels(\"" + xAxisLabel + "\", \"" + yAxisLabel + "\");\n");
			Recorder.recordString(
					plotDot + "setFormatFlags(" + (Recorder.scriptMode() ? "0x" + Integer.toHexString(flags)
	...
	private void doHighResolutionDialog(Shell parent) {
		...
		if (IJ.recording()) {
			if (Recorder.scriptMode()) {
				Recorder.recordCall("plot.makeHighResolution(\"" + title + "\"," + hiResFactor + "," + hiResAntiAliased
						+ ",true);");
			} else {
	 ...
	 */
	void enableDisableLogCheckbox(org.eclipse.swt.widgets.Button checkbox, double limit1, double limit2) {
