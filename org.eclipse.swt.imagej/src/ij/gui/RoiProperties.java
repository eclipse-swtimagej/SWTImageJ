 ...
 */
public class RoiProperties implements ModifyListener, ShellListener {
	...
	public boolean showDialog() {
		...
		groupName = (org.eclipse.swt.widgets.Label)gd.getMessage();
		if(showName && !IJ.isMacro()) {
			Vector v = gd.getStringFields();
			groupField = (Text)v.elementAt(v.size() - 2);
			/* Changed for SWT! */
			...
			dis.syncExec(new Runnable() {
				...
				public void run() {
					...
					groupField.addModifyListener(RoiProperties.this);
				}
			});
			colorField = (Text)v.elementAt(v.size() - 1);
		}
		if(!isLine) {
			if(isPoint) {
		...
		if(overlayOptions) {
			...
			if(overlay != null) {
				...
				if(imp != null && imp.getHideOverlay())
					gd.addMessage("Current overlay is hidden", null, ij.swt.Color.darkGray);
				else
					gd.addMessage("Current overlay has " + size + " element" + (size > 1 ? "s" : ""), null, ij.swt.Color.darkGray);
				gd.setInsets(0, 30, 0);
				...
				gd.addMessage("No overlay", null, ij.swt.Color.darkGray);
		}
		if(isText)
		...
		dis.syncExec(new Runnable() {
			...
			public void run() {
				...
				gd.getShell().addShellListener(RoiProperties.this);
				gd.showDialog();
			}
		});
		if(gd.wasCanceled())
