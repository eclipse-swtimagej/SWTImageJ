 ...
 */
public class PlotWindow extends ImageWindow implements ImageListener, ClipboardOwner, RoiListener, Runnable {
	...
	private org.eclipse.swt.widgets.Button list, data, more, live;
	org.eclipse.swt.widgets.Menu dataPopupMenu;
	org.eclipse.swt.widgets.Menu morePopupMenu;
	// private org.eclipse.swt.widgets.PopupMenu dataPopupMenu, morePopupMenu;
	private static final int NUM_MENU_ITEMS = 20; // how many menu items we have in total
	private org.eclipse.swt.widgets.MenuItem[] menuItems = new org.eclipse.swt.widgets.MenuItem[NUM_MENU_ITEMS];
	...
	public Composite bottomPanel;
	private org.eclipse.swt.graphics.Font font;
	private org.eclipse.swt.graphics.Font fontLivePlot;
	...
	public void draw() {
		...
		bottomPanel = new Composite(super.parentComposite, SWT.NONE);
		bottomPanel.setLayoutData(new GridData(SWT.FILL, SWT.FILL, false, false, 1, 1));
		GridLayout gl_composite = new GridLayout(6, false);
		gl_composite.verticalSpacing = 0;
		gl_composite.horizontalSpacing = 0;
		gl_composite.marginHeight = 0;
		gl_composite.marginWidth = 0;
		bottomPanel.setLayout(gl_composite);
		// int hgap = IJ.isMacOSX() ? 1 : 5;
		list = new org.eclipse.swt.widgets.Button(bottomPanel, SWT.NONE);
		...
		data = new org.eclipse.swt.widgets.Button(bottomPanel, SWT.NONE);
		data.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false, false, 1, 1));
		data.setText(dataButtonLabel);
		...
		more = new org.eclipse.swt.widgets.Button(bottomPanel, SWT.NONE);
		more.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false, false, 1, 1));
		...
		statusLabel = new Label(bottomPanel, SWT.NONE);
		statusLabel.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, true, 2, 1));
		statusLabel.setText("");
		...
		getDataPopupMenu(data);
		getMorePopupMenu(more);
		plot.draw();
		/*
		 * LayoutManager lm = getLayout(); if (lm instanceof ImageLayout) ((ImageLayout)
		 * lm).ignoreNonImageWidths(true); // don't expand size to make the panel fit //
	...
	void maximizeCoordinatesLabelWidth() {
		 ...
		 * Composite parent = statusLabel.getParent(); // the bottomPanel, has insets of
		 * 0 //if (!parent.isValid()) // parent.validate(); int cWidth = getWidth() - 2
		 * * HGAP - statusLabel.x; int cHeight = statusLabel.getPreferredSize().height;
	 ...
	 * @param data
	 */
	org.eclipse.swt.widgets.Menu getDataPopupMenu(Button data) {

		dataPopupMenu = new org.eclipse.swt.widgets.Menu(data);
		data.setMenu(dataPopupMenu);
		data.addSelectionListener(new SelectionAdapter() {
		...
		menuItems[SAVE] = addSwtPopupItem(dataPopupMenu, "Save Data...");
		menuItems[COPY] = addSwtPopupItem(dataPopupMenu, "Copy 1st Data Set");
		menuItems[COPY_ALL] = addSwtPopupItem(dataPopupMenu, "Copy All Data");
		menuItems[LIST_SIMPLE] = addSwtPopupItem(dataPopupMenu, "List (Simple Headings)");
		new org.eclipse.swt.widgets.MenuItem(dataPopupMenu, SWT.SEPARATOR);
		menuItems[ADD_FROM_TABLE] = addSwtPopupItem(dataPopupMenu, "Add from Table...");
		menuItems[ADD_FROM_PLOT] = addSwtPopupItem(dataPopupMenu, "Add from Plot...");
		menuItems[ADD_FIT] = addSwtPopupItem(dataPopupMenu, "Add Fit...");
		return dataPopupMenu;
	}

	/** Prepares and returns the popupMenu of the More>> button */
	org.eclipse.swt.widgets.Menu getMorePopupMenu(Button more) {

		org.eclipse.swt.widgets.Menu morePopupMenu = new org.eclipse.swt.widgets.Menu(more);
		more.setMenu(morePopupMenu);
		more.addSelectionListener(new SelectionAdapter() {

			@Override
			public void widgetSelected(SelectionEvent e) {

				morePopupMenu.setVisible(true);
			}
		});
		// morePopupMenu = new PopupMenu();
		// GUI.scalePopupMenu(morePopupMenu);
		menuItems[SET_RANGE] = addSwtPopupItem(morePopupMenu, "Set Range...");
		menuItems[PREV_RANGE] = addSwtPopupItem(morePopupMenu, "Previous Range");
		menuItems[RESET_RANGE] = addSwtPopupItem(morePopupMenu, "Reset Range");
		menuItems[FIT_RANGE] = addSwtPopupItem(morePopupMenu, "Set Range to Fit All");
		menuItems[ZOOM_SELECTION] = addSwtPopupItem(morePopupMenu, "Zoom to Selection");
		new org.eclipse.swt.widgets.MenuItem(morePopupMenu, SWT.SEPARATOR);
		menuItems[AXIS_OPTIONS] = addSwtPopupItem(morePopupMenu, "Axis Options...");
		menuItems[LEGEND] = addSwtPopupItem(morePopupMenu, "Legend...");
		menuItems[STYLE] = addSwtPopupItem(morePopupMenu, "Contents Style...");
		menuItems[TEMPLATE] = addSwtPopupItem(morePopupMenu, "Use Template...");
		menuItems[RESET_PLOT] = addSwtPopupItem(morePopupMenu, "Reset Format");
		menuItems[FREEZE] = addSwtPopupItem(morePopupMenu, "Freeze Plot", true);
		menuItems[HI_RESOLUTION] = addSwtPopupItem(morePopupMenu, "High-Resolution Plot...");
		new org.eclipse.swt.widgets.MenuItem(morePopupMenu, SWT.SEPARATOR);
		menuItems[PROFILE_PLOT_OPTIONS] = addSwtPopupItem(morePopupMenu, "Plot Defaults...");
	...
	org.eclipse.swt.widgets.MenuItem addSwtPopupItem(org.eclipse.swt.widgets.Menu popupMenu, String s, boolean isCheckboxItem) {
		...
		if(isCheckboxItem) {
			...
			item = new org.eclipse.swt.widgets.MenuItem(popupMenu, SWT.NONE);
	...
	void copyToClipboard(boolean writeAllColumns) {
		...
		/*
		 * Clipboard systemClipboard = null; try { systemClipboard =
		 * getToolkit().getSystemClipboard(); } catch (Exception e) { systemClipboard =
		 * null; }
		 */
		/*
		 * if (cb == null) { IJ.error("Unable to copy to Clipboard."); return; }
		 */
	...
	private void enableLivePlot() {
		...
		live.setFont(font);
		live.setForeground(IJ.getInstance().getDisplay().getSystemColor(SWT.COLOR_RED));
	}
	...
	private void disableLivePlot() {
		...
		if(live != null) {
			if(live.isDisposed() == false) {
				...
				org.eclipse.swt.graphics.Font liveFont = live.getFont();
				FontData[] fontData = liveFont.getFontData();
				fontLivePlot = new org.eclipse.swt.graphics.Font(liveFont.getDevice(), fontData);
				live.setFont(fontLivePlot);
				live.setForeground(Display.getDefault().getSystemColor(SWT.COLOR_BLACK));
			}
		}
	}
