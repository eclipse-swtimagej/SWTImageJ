...
public class PlotContentsDialog implements DialogListener {
	...
	private org.eclipse.swt.widgets.Combo        objectChoice;
	private org.eclipse.swt.widgets.Combo        symbolChoice;
	private org.eclipse.swt.widgets.Text     colorField, color2Field, labelField, widthField;
	private org.eclipse.swt.widgets.Button      visibleCheckbox;
	private boolean       creatingPlot;              // for creating plots; dialogType determines source
	private org.eclipse.swt.widgets.Combo        plotChoice;                // for "Add from Plot"
	...
	private org.eclipse.swt.widgets.Combo        tableChoice;               // for "Add from Table"
	...
	private org.eclipse.swt.widgets.Combo[]      columnChoice = new org.eclipse.swt.widgets.Combo[N_COLUMNS];
	...
	private org.eclipse.swt.widgets.Combo        fitDataChoice;             // for "Add Fit"
	private org.eclipse.swt.widgets.Combo        fitFunctionChoice;
	...
	public void showDialog(Shell parent) {
		...
		if (parent == null && plot.getImagePlus() != null)
			parent = plot.getImagePlus().getWindow().shell;
		...
		if (dialogType == STYLE) {
			...
			objectChoice = (org.eclipse.swt.widgets.Combo)(gd.getChoices().get(0));
			currentObjectIndex = objectChoice.getSelectionIndex();
		} else if (dialogType == ADD_FROM_PLOT) {
			...
			plotChoice = (org.eclipse.swt.widgets.Combo)(choices.get(0));
			objectChoice = (org.eclipse.swt.widgets.Combo)(choices.get(1));
			...
			tableChoice = (org.eclipse.swt.widgets.Combo)(gd.getChoices().get(0));
			...
			Vector choices = gd.getChoices();
			fitDataChoice = (org.eclipse.swt.widgets.Combo)(choices.get(0));
			fitFunctionChoice = (org.eclipse.swt.widgets.Combo)(choices.get(1));
			if (dataSources.length == 1)
				fitDataChoice.setEnabled(false);
			for (int i=0; i<CurveFitter.fitList.length; i++)
				fitFunctionChoice.add(CurveFitter.fitList[CurveFitter.sortedTypes[i]]);
			fitFunctionChoice.setText(lastFitFunction);
			/*Select a default combo item for SWT. Else the selected index is -1!*/
			fitFunctionChoice.select(0);
		}
		if (dialogType == ADD_FROM_TABLE || dialogType == ADD_FROM_ARRAYS) {
			for (int i=0; i<nColumnsToUse; i++) {
				...
				Vector choices = gd.getChoices();
				columnChoice[i] = (org.eclipse.swt.widgets.Combo)(choices.get(choices.size()-1));
			}
			makeSourceColumns();
			if (!creatingPlot)
				suggestNewYColumn();
		...
		Vector choices = gd.getChoices();
		symbolChoice = (org.eclipse.swt.widgets.Combo)(choices.get(choices.size()-1));
		Vector stringFields = gd.getStringFields();
		colorField = (org.eclipse.swt.widgets.Text)(stringFields.get(0));
		color2Field = (org.eclipse.swt.widgets.Text)(stringFields.get(1));
		labelField = (org.eclipse.swt.widgets.Text)(stringFields.get(2));
		widthField = (org.eclipse.swt.widgets.Text)(gd.getNumericFields().get(0));
		visibleCheckbox = (org.eclipse.swt.widgets.Button)(gd.getCheckboxes().get(0));
		gd.addDialogListener(this);
		IJ.wait(100);			//sometimes needed to avoid hanging?
		if (dialogType == STYLE)
			setDialogStyleFields(objectChoice.getSelectionIndex());
		else if (dialogType == ADD_FROM_PLOT)
			addObjectFromPlot();
		...
		if (dialogType == ADD_FROM_TABLE || dialogType == ADD_FROM_ARRAYS || dialogType == ADD_FIT) {
			...
			previousSymbol = symbolChoice.getItem(symbolChoice.getSelectionIndex());
			previousLineWidth = Tools.parseDouble(widthField.getText());
		}
		if (dialogType == ADD_FIT) {
			lastFitFunction = fitFunctionChoice.getItem(fitFunctionChoice.getSelectionIndex());
			IJ.log(curveFitterStatusString);
		}
		if (IJ.recording() && !Recorder.scriptMode()) {
			if (dialogType == ADD_FROM_PLOT) {
				Recorder.recordString("Plot.addFromPlot(\""+plotChoice.getItem(plotChoice.getSelectionIndex())+"\", "+objectChoice.getSelectionIndex()+");\n");
			} else if (dialogType == ADD_FROM_TABLE) {
				if (creatingPlot)
					Recorder.recordString("Plot.create(\""+plot.getTitle()+"\", \""+plot.getLabel('x')+"\", \""+plot.getLabel('y')+"\");\n");
				String xSource = columnChoice[0].getSelectionIndex() == 0 ? "" :
						"Table.getColumn(\""+columnChoice[0].getItem(columnChoice[0].getSelectionIndex())+"\", \""+tableChoice.getItem(tableChoice.getSelectionIndex())+"\"), ";
				String ySource = "Table.getColumn(\""+columnChoice[1].getItem(columnChoice[1].getSelectionIndex())+"\", \""+tableChoice.getItem(tableChoice.getSelectionIndex())+"\")";
				Recorder.recordString("Plot.add(\""+symbolChoice.getItem(symbolChoice.getSelectionIndex())+"\", "+xSource+ySource+");\n");
				if (columnChoice[2].getSelectionIndex() > 0)
					Recorder.recordString("Plot.add(\"xerror\", Table.getColumn(\""+
							columnChoice[2].getItem(columnChoice[2].getSelectionIndex())+"\", \""+tableChoice.getItem(tableChoice.getSelectionIndex())+"\"));\n");
				if (columnChoice[3].getSelectionIndex() > 0)
					Recorder.recordString("Plot.add(\"error\", Table.getColumn(\""+
							columnChoice[3].getItem(columnChoice[3].getSelectionIndex())+"\", \""+tableChoice.getItem(tableChoice.getSelectionIndex())+"\"));\n");
			}
	...
	public boolean dialogItemChanged(GenericDialog gd, TypedEvent e) {
		...
		if (dialogType == STYLE) {
			int objectIndex = objectChoice.getSelectionIndex();  // no getNextChoice since Choices depend on dialog type
	...
	private String getStyleString() {
		...
		String symbol = symbolChoice.getItem(symbolChoice.getSelectionIndex());
		Boolean visible = visibleCheckbox.getSelection();
	...
	private void setDialogStyleFields(int index) {
		if (index < 0) return;
		org.eclipse.swt.widgets.Button visibleC = (org.eclipse.swt.widgets.Button)gd.getCheckboxes().get(0);
		...
		if (items.length >= 4)
			symbolChoice.setText(items[3]);
		String olabel=plot.getPlotObjectLabel(index);
		if(olabel==null) {
			olabel="";
		}
		labelField.setText(isData ? olabel : "");
		visibleC.setSelection(!styleString.contains("hidden"));
	...
	private void makeSourcePlotObjects() {
		int plotIndex = plotChoice.getSelectionIndex();
		...
		for (int i=0; i<nPlotObjects; i++)
			objectChoice.add(plotObjectNames[i]);
	...
	private void addObjectFromPlot() {
		int plotIndex = plotChoice.getSelectionIndex();
		int objectIndex = objectChoice.getSelectionIndex();
		plot.restorePlotObjects();
		if (savedLimits != null)
	...
	private void prepareAddFromTable() {
		...
		Object[] windows = WindowManager.getNonImageWindows();
		...
		for (int i=0; i<allTables.length; i++) {
			...
			if (allTableNames[i]!=null && allTableNames[i].equals(previousTableName))
				defaultTableIndex = i;
			allTableNames[i] = tw.getShell().getText();
		}
	}
	...
	private void makeSourceColumns() {
		...
		if (dialogType == ADD_FROM_TABLE) {
			int tableIndex = tableChoice.getSelectionIndex();
		...
		for (int c=0; c<nColumnsToUse; c++) {
			...
			for (int i=COLUMN_ALLOW_NONE[c] ? 0 : 1; i<columnHeadings.length; i++)
					columnChoice[c].add(columnHeadings[i]);
	...
	private void addObjectFromTable() {
		...
		String label = columnChoice[1].getItem(columnChoice[1].getSelectionIndex());	//take label from y
		int shape = Plot.toShape(symbolChoice.getItem(symbolChoice.getSelectionIndex()));
		...
		if (creatingPlot) {
			plot.setXYLabels(data[0]==null ? "x" : columnChoice[0].getItem(columnChoice[0].getSelectionIndex()), columnChoice[1].getItem(columnChoice[1].getSelectionIndex()));
		...
		setDialogStyleFields(currentObjectIndex);
		if (dialogType == ADD_FROM_TABLE)
			previousTableName = allTableNames[tableChoice.getSelectionIndex()];
	}
	...
	private void suggestNewYColumn() {
		...
		int currentIndex = columnChoice[1].getSelectionIndex();
		for (int i=0; i<nYcolumns; i++) {
			if (columnChoice[0].getSelectionIndex()==0 ||    //if x is a data column, don't suggest it as y
					!columnChoice[1].getItem(columnChoice[1].getSelectionIndex()).equals(columnChoice[0].getItem(columnChoice[0].getSelectionIndex()))) {
				float[][] data = getDataArrays();
	...
	private float[][] getDataArrays() {
		...
		if (dialogType == ADD_FROM_TABLE) {
			ResultsTable rt = allTables[tableChoice.getSelectionIndex()];
			for (int c=0; c<N_COLUMNS; c++) {
				String heading = columnChoice[c].getItem(columnChoice[c].getSelectionIndex());
				int index = rt.getColumnIndex(heading);
				if (index >= 0)
					data[c] = rt.getColumn(index);
				previousColumns[c] = columnChoice[c].getSelectionIndex();
			}
		} else { //if (dialogType == ADD_FROM_ARRAYS)
			for (int c=0; c<nColumnsToUse; c++) {
				String heading = columnChoice[c].getItem(columnChoice[c].getSelectionIndex());
	...
	private void addFitCurve() {
		...
		if (savedLimits != null)
			plot.setLimits(savedLimits);
		int dataIndex = fitDataChoice.getSelectionIndex();
		float[][] data = plot.getDataObjectArrays(dataIndex);
		String fitName = fitFunctionChoice.getItem(fitFunctionChoice.getSelectionIndex());
		...
		curveFitterStatusString = "Fit for "+plot.getTitle()+": "+fitDataChoice.getItem(fitDataChoice.getSelectionIndex())+cf.getResultString(); //will be shown in Log when done
