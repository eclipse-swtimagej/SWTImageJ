...
public class Arrow extends Line {
	...
	public void draw(Graphics g) {
		...
		at.setTransform(mag * ic.aspectRatioX, 0.0, 0.0, mag * ic.aspectRatioY, (-xbase+0.5)*mag * ic.aspectRatioX, (-ybase+0.5)*mag * ic.aspectRatioY); //0.5: int coordinate at pixel center
		if (outline) {
		...
		if (!overlay) {
			...
			drawHandle(g, screenXD(x1d* ic.aspectRatioX), screenYD(y1d* ic.aspectRatioY));
			drawHandle(g, screenXD(x2d* ic.aspectRatioX), screenYD(y2d* ic.aspectRatioY));
			drawHandle(g, (int)(screenXD(x1d+(x2d-x1d)/2.0)*ic.aspectRatioX), (int)(screenYD(y1d+(y2d-y1d)/2.0)*ic.aspectRatioY));
		}
