...
public class PlotCanvas extends ImageCanvas {
	...
	public void fitToWindow() {
		...
		ImageWindow win = imp.getWindow();
		if (win==null) return;
		Rectangle bounds = win.getSwtParentBounds();
	...
	public void setSourceRect(Rectangle r) {
		if (plot.isFrozen()) {
		Rectangle recSwt = new Rectangle(r.x, r.y, r.width, r.height);
			super.setSourceRect(recSwt);
		}
		else
			resetMagnification();
	}
    ...
    int getRangeArrowIndex(org.eclipse.swt.events.MouseEvent e) {
        ImageWindow win = imp.getWindow();
    ...
    /*
	 * Changed for SWT! We have no visible PlotWindow so we implement the mouse
	 * listener here!
	 */
