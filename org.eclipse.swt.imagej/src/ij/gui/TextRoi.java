...
public class TextRoi extends Roi {
	...
	*/
	public void drawPixels(ImageProcessor ip) {
		...
		int i = 0;
		int yy = 0;
		/*Also calculate aspectRation. 1.0 for the normal image view!*/
		ic.calculateAspectRatio();
		int xi = (int)Math.round(getXBase()*ic.aspectRatioX);
		int yi = (int)Math.round(getYBase()*ic.aspectRatioY);
	...
	public void draw(Graphics g) {
		...
		int sx = screenXD(getXBase()*aspectRatioX);
		int sy = screenYD(getYBase()*aspectRatioY);
		int swidth = (int)((bounds!=null?bounds.width:this.width)*mag*aspectRatioX);
		int sheight = (int)((bounds!=null?bounds.height:this.height)*mag*aspectRatioY);
		Rectangle r = null;
		if (angle!=0.0)
			drawText(g);
		else {
	...
	void drawText(Graphics g) {
		...
		int xi = (int)Math.round(getXBase()*aspectRatioX);
		int yi = (int)Math.round(getYBase()*aspectRatioY);
		...
		/*Also calculate aspectRation. 1.0 for the normal image view!*/
		int sx = nonScalable?xi:screenXD(getXBase()*aspectRatioX);
		int sy = nonScalable?yi:screenYD(getYBase()*aspectRatioY);
		int sw = nonScalable?widthi:(int)(getMagnification()*widthd*aspectRatioX);
		int sh = nonScalable?heighti:(int)(getMagnification()*heightd*aspectRatioY);
