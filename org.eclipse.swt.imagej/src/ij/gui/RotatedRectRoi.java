...
public class RotatedRectRoi extends PolygonRoi {
	...
	public RotatedRectRoi(int sx, int sy, ImagePlus imp) {
		...
		int pixels = win != null ? (int)(win.getSize().x / win.getCanvas().getMagnification()) : imp.getHeight();
		if(IJ.debugMode)
	...
	public int isHandle(int sx, int sy) {
		...
		int index = -1;
		for(int i = 0; i < 4; i++) {
			int sx2 = (int)Math.round(hxs(i) - halfSize),
					sy2 = (int)Math.round(hys(i) - halfSize);
