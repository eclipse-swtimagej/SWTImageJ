...
public class PolygonRoi extends Roi {
	...
	long mouseUpTime = 0;
	/* Extra temp spline arrays for a anisotropic view! */
	private float[] xSplineTemp;
	private float[] ySplineTemp;
	...
	public void draw(Graphics g) {
		...
		if(xSpline != null) {
			xSplineTemp = new float[xSpline.length];
			ySplineTemp = new float[ySpline.length];
			/* Scale according to aspect ratio! */
			if(ic.isFitToParent()) {
				ic.calculateAspectRatio();
				for(int i = 0; i < xSpline.length; i++) {
					/*
					 * When we draw we have to store the values in a temp array! Else we constantly
					 * multiply the spline arrays!
					 */
					xSplineTemp[i] = xSpline[i] * ic.aspectRatioX;
					ySplineTemp[i] = ySpline[i] * ic.aspectRatioY;
				}
				for(int i = 0; i < xp2.length; i++) {
					xp2[i] = (int)(xp2[i] * ic.aspectRatioX);
					yp2[i] = (int)(yp2[i] * ic.aspectRatioY);
				}
				/* Important to rescale to the normal ImageJ view (isotropic)! */
			} else {
				ic.calculateAspectRatio();
				for(int i = 0; i < xSpline.length; i++) {
					xSplineTemp[i] = xSpline[i] * ic.aspectRatioX;
			...
			if(type == POLYLINE || type == FREELINE) {
				...
				/* Draw with the temp array for the aspect ratio! */
				drawSpline(g, xSplineTemp, ySplineTemp, splinePoints, false, fill, isActiveOverlayRoi);
				if(wideLine && !overlay) {
			...
			} else /* Draw with the temp array for the aspect ratio! */
				drawSpline(g, xSplineTemp, ySplineTemp, splinePoints, true, fill, isActiveOverlayRoi);
		} else {
			/* Scale according to aspect ratio! */
			if(ic.isFitToParent()) {
				ic.calculateAspectRatio();
			...
			if(type == POLYLINE || type == FREELINE || type == ANGLE || (state == CONSTRUCTING && !(this instanceof RotatedRectRoi))) {
	...
	private void drawSpline(Graphics g, float[] xpoints, float[] ypoints, int npoints, boolean closed, boolean fill, boolean isActiveOverlayRoi) {
		...
		/* Calculate the aspect ratio for an anisotropic view! */
		double xd = getXBase() * ic.aspectRatioX;
		double yd = getYBase() * ic.aspectRatioY;
		Graphics2D g2d = (Graphics2D)g;
	...
	public void mouseMoved(MouseEvent e) {
		...
		ic.calculateAspectRatio();
		int sx = (int)(e.getX() / ic.aspectRatioX);
		int sy = (int)(e.getY() / ic.aspectRatioY);
		// int sx = e.getX();
		// int sy = e.getY();
		...
		if(tool == Toolbar.ANGLE) {
			if(nPoints == 2)
				...
				double angle2 = xpf != null ? getFloatAngle(xpf[1], ypf[1], xpf[2], ypf[2]) : getAngle(xp[1], yp[1], xp[2], yp[2]);
	...
	void drawRubberBand(int sx, int sy) {
		...
		int xmin = Integer.MAX_VALUE, ymin = Integer.MAX_VALUE, xmax = 0,
				ymax = 0;
		if(x1 < xmin)
			xmin = x1;
		if(x2 < xmin)
	...
	void finishPolygon() {
		...
		oldWidth = width;
		oldHeight = height;
		if(Recorder.record && userCreated && (type == POLYGON || type == POLYLINE || type == ANGLE || (type == POINT && Recorder.scriptMode() && nPoints == 3)))
	...
	protected void handleMouseUp(int sx, int sy) {
		...
		if(nPoints > 2 && (biggerStartBox.contains(sx, sy) || (offScreenXD(sx) == startXD && offScreenYD(sy) == startYD) || (samePoint && doubleClick))) {
	...
	public int isHandle(int sx, int sy) {
		...
		int sx2, sy2;
		for(int i = 0; i < nPoints; i++) {
			/* Also consider aspect ration for the handle selection! */
			sx2 = ((int)(xp2[i] / ic.aspectRatioX)) - halfSize;
			sy2 = ((int)(yp2[i] / ic.aspectRatioY)) - halfSize;
	...
	/**
	 * Returns 'true' if this ROI was created using the wand tool or the particle
	 * analyzer.
	 */
	...
	static double getLength(int[] xpoints, int[] ypoints, int npoints, boolean closeShape, ImagePlus imp) {
		...
		for(int i = 0; i < npoints - 1; i++)
			length += Math.sqrt(sqr((xpoints[i + 1] - xpoints[i]) * pixelWidth) + sqr((ypoints[i + 1] - ypoints[i]) * pixelHeight));
		if(closeShape)
			length += Math.sqrt(sqr((xpoints[0] - xpoints[npoints - 1]) * pixelWidth) + sqr((ypoints[0] - ypoints[npoints - 1]) * pixelHeight));
		return length;
	}
